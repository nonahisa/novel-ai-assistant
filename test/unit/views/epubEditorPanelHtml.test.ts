...
describe("右の縦の並び（設計書6.65.15の段D）", () => {
  ...
  /**
   * 保留の面の呼び名（設計書5.4節・6.65.15の段D）。
   *
   * 保留の状態そのもの（`suspendBlock` の送受信）は見張られているが、
   * 並びの行に添える文言はこれまで見張られていなかった
   * （`grep "保留中：本には入りません"` で当たるのは featureGuide.test.ts だけ）。
   */
  it("保留にした面は、並びの行の呼び名に「保留中：本には入りません」が付く", () => {
...
describe("選んだ面のプレビュー", () => {
  ...
  /**
   * 保留の面のプレビュー（設計書5.4節・6.65.15の段D）。
   *
   * プレビューは保留の面も見せるが、見出しに「保留中：本には入りません」を
   * 付けて、本には入らないことをその場で言う。保留の状態の受け渡し
   * （`suspendBlock`）は見張られているが、この見出しの文言はこれまで
   * 見張られていなかった。
   */
  it("保留の面は、見出しに「保留中：本には入りません」が付く", () => {
    expect(script).toContain(
      "(page.suspended ? '（保留中：本には入りません）' : '')"
    );
  });

  it("保留でない面には、見出しに何も足さない", () => {
    // 上のテストと対で見る。三項演算子の否側が空文字であることを
    // 直に確かめないと、「常に付く」に変わっても気づけない
    expect(script).not.toContain(
      "escapeHtml(page.label) + '（保留中：本には入りません）'"
    );
  });

  /**
   * 焼いたあとに欄を変えた表紙（精査 R19）。見本を描き直している面には、
   * 見出しに「まだ焼いていません」を付ける（本に入るのは前に焼いた画像）。
   * どの面に付けるかは拡張機能側が `unbaked` で決める
   * （`epubEditorPanelPreview.test.ts`）。
   */
  it("見本を描き直している面は、見出しに「まだ焼いていません」が付く", () => {
    expect(script).toContain("(page.unbaked ? '（まだ焼いていません）' : '')");
  });
});

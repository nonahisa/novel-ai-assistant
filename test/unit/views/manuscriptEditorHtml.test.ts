...
describe("下段の字数", () => {
  ...
  it("3つとも出す", () => {
    ...
    expect(code).toContain('"この話で今日 "');
  });

  /**
   * 「今日」は**この話で**今日書いた量であって、作品全体の今日の量ではない
   * （ノートPCの実機確認、2026-09-25）。「今日 0字」と「2日連続で目標に
   * 届きました」が同じ下段に並ぶと、今日は何も書いていないのに届いたと
   * 読めてしまう（届いたのは別の話で書いた分）。
   */
  it("今日の字数は「この話で」と断る（作品全体の今日の量と取り違えない）", () => {
    expect(code).not.toMatch(/"今日 " \+/);
  });

...
describe("作品情報を設定資料集で見られる", () => {
  ...
  test("まだ無いときは、どこで作れるかを書く", () => {
    ...
    // 道順は分類から書く（各話あらすじは資料管理、紹介文・キャッチコピーは
    // 投稿脱稿。2026-09-24 に「執筆支援」を工程の束へ割った）
    expect(script()).toContain("詳細メニューの「");
    expect(script()).toContain("資料管理 → 資料抽出 → 各話あらすじ");
    expect(script()).toContain("投稿脱稿 → 広報支援 → ");
  });
...
/**
 * 参考の行に添える操作（2026-09-21）。
 *
 * 「変化かもしれない」の行には昇格の操作があり、「変化」の行には
 * 誤りを落とす操作が付いた。**どちらを送るかは拡張機能側が決める。**
 */
describe("参考の行のボタン", () => {
  test("行が指定した種別をそのまま送る", () => {
    expect(script()).toContain("entry.action.kind");
    expect(script()).toContain("post(messageType");
  });

  test("古い画面から来ても、昇格として扱う", () => {
    // 種別を持たない行（以前の形）が届いても押せる
    expect(script()).toContain('|| "promoteConflict"');
  });
});

...
describe("「できること」から機能を起動する", () => {
  ...
  /*
    **この3件は、もとは `toContain("（AIを使います）")` の1行だった**
    （2026-09-22 に強くした）。あの形では**束のどこかにその字があれば通る**。
    製品側にはこの字が**3か所**あるので、**1か所で条件を取り違えても、
    逆にしても通ってしまう**——肝心の「**AIを使わない操作には付かない**」が
    見張られていなかった。

    **3か所は役割が違う。**（1）押せる操作の並び（条件つき）（2）「できること」の
    一覧（使う・使わないを言い分ける）（3）「AIで再読込」（**必ず使うので
    条件なし**）。**3つ目があるので「どこに出てもよい」とは書けない。**
  */
  test("押す前の印は、AIを使う操作にだけ付く（条件ごと見張る）", () => {
    // クラウドのAIは実行のたびに課金される。**条件を落とすと、
    // 使わない操作にも「（AIを使います）」が付いて嘘になる**
    expect(script(LARGE)).toContain(
      "run.usesAI ? '（AIを使います）' : ''"
    );
  });

  test("「できること」の一覧では、使う・使わないを言い分ける", () => {
    // ここは空文字ではなく「（AIを使いません）」と言い切る。
    // **逆に書くと、無料の操作が有料に見える**
    expect(script(LARGE)).toContain(
  ...
  test("「AIで再読込」は必ずAIを使うので、条件を付けない", () => {
    // **3か所目がこれ。** 「どこに出てもよい」と書けない理由でもある
    expect(script(LARGE)).toContain("'（AIを使います）</span>'");
  });

...
describe("novel.propose（人物以外）——断りどころ", () => {
  ...
  /*
    作者の裁定（2026-09-24「３ＯＫ」）：人物の道と揃え、作者が確定させた記録にも
    置く。反映は作者がパネルで採ったときだけなので、確定の記録を外から
    黙って書き換えることにはならない。台帳は置いた時点では変わらない。
  */
  it("作者が確定させた記録（autoGenerated: false）にも置く。台帳は変わらず、確定の印は写しでも false のまま", () => {
    const folder = workCopy();
    ...
    expect(result).toMatchObject({ recordKind: "location", source: "external" });
    const files = fs.readdirSync(pendingSettingsDir(folder));
    expect(files).toEqual(["loc_001.json"]);
    const payload = parsePendingSettingsPayload(
      readJson(nodePath.join(pendingSettingsDir(folder), files[0]))
    );
    expect(payload.source).toBe("external");
    expect(payload.record.autoGenerated).toBe(false);
    expect(ledgerFingerprint(folder)).toBe(before);
  });
...
describe("novel.propose（場所）→ 更新分を反映", () => {
  ...
  it("作者が確定させた場所でも、承認すると説明が入る。確定の印は false のまま、作者メモは残る", async () => {
    const folder = workCopy();
    const { file } = seedLocation(folder, { autoGenerated: false });
    novelPropose({
      folder,
      recordKind: "location",
      name: "港町",
      changes: { description: "灯台と魚市場のある港", aliases: ["魚の港"] },
      reason: "第3話で魚市場の場面があるため。",
    });
    ...
    const captured = {
      items: [] as Array<{ id: string; changes: string[] }>,
      apply: undefined as
        | ((id: string) => Promise<{ ok: boolean; reason?: string }>)
        | undefined,
    };
    ...
    // 出口が「古い案」として黙って片付けていないこと（確定の記録で起きていた形）
    expect(captured.items).toHaveLength(1);
    expect(captured.items[0].changes.join("\n")).toContain("灯台と魚市場のある港");
    ...
    const saved = readJson(file);
    expect(saved.description).toBe("灯台と魚市場のある港");
    expect(saved.aliases).toEqual(["灯の港", "魚の港"]);
    expect(saved.autoGenerated).toBe(false);
    expect(saved.authorNotes).toBe("作者の覚え");
    expect(saved.exportNote).toBe("資料用の補足");
    expect(fs.readdirSync(pendingSettingsDir(folder))).toEqual([]);
  });
});

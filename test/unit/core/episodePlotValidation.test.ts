 ...
 */
/**
 * 箇条書きの欄に行を書いたうえで、**引用の欄にも同じ行を写してくる**
 * （2026-09-26 の逸脱の測り直し、さくら Kimi-K2.6・教科書チートの第3話）。
 * 助ける人を入れ替えた仕込みの行に「主筋の改変」を付け、理由は「炎槍を放ったのは
 * ターナではなくマイナ」と正しく言い当てていたのに、引用が本文に無いとして落ちた。
 * 行は指せているので、**写しただけの引用を外して**指摘は残す（本文に無い字は見せない）。
 */
describe("P-28 箇条書きの行を、箇条書きの欄と引用の欄の両方に写した答え", () => {
  test("引用を外して指摘は残す（Kimi の実物の形）", () => {
    const { accepted, rejected } = contrast([
      {
        kind: "主筋の改変",
        plotItem: "老人が訪ねてくる",
        excerpt: "老人が訪ねてくる",
        reason: "訪ねてくるのは老人ではなく少女である。",
      },
    ]);
  ...
  test("引用が別の行の写しなら、これまでどおり本文に無い引用として捨てる", () => {
    const { accepted, rejected } = contrast([
      {
        kind: "主筋の改変",
        plotItem: "老人が訪ねてくる",

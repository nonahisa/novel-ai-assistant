...
describe("使い方の節も、目次と一緒に外す", () => {
  ...
  */
  ...
  /*
    ...
    **7,033字へ更新した（2026-09-23・v3.16）。** 起動できる機能の札へ
    「応募先をAIに提案してもらう」（設計書6.3.6.5）の1件を足したぶん（41字）
    増えている。v3.14 と同じく、**埋め込む一覧が動いた側**の更新である。
  */
  test("目次を渡す回のシステム指示は、割る前と同じ長さ", () => {
    expect(WORK_CHAT_SYSTEM_PROMPT.length).toBe(7033);
  });

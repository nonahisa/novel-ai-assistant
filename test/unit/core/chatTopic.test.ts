...
describe("使い方の節も、目次と一緒に外す", () => {
  ...
  */
  ...
  /*
    ...
    **7,082字へ更新した（2026-09-23・v3.17）。** v3.16 で起動できる機能の札へ
    「応募先をAIに提案してもらう」（設計書6.3.6.5）の1件を足したぶん（41字）と、
    v3.17 で【出力形式】の例に `, "writerStyleSignals": null`（28字）を足し、断り書きを
    「profileSignals と writerStyleSignals は」（21字増）にしたぶん。

    **7,268字へ更新した（2026-09-24・v3.18）。** 【答え方】へ、講評を求められた
    ときの構え（良いところを先に引用と理由つきで・直す所が無ければそう書いて
    よい。プロンプト設計書1.9）の1項目を足したぶん（186字）。

    **7,465字へ更新した（2026-09-24・v3.20）。** 使い方の節へ、相談パネルへ
    バックアップを落とす入口（隠し機能。聞かれたときだけ案内し自分から勧めない）の
    1項目を足したぶん（197字）。
  */
  test("目次を渡す回のシステム指示は、割る前と同じ長さ", () => {
    expect(WORK_CHAT_SYSTEM_PROMPT.length).toBe(7465);
  });

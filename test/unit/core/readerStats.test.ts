...
describe("読者の反応の台帳", () => {
  ...
  /*
    ...
    **なぜ要るか**：「3つの輪」の `collectReactions`（0.82.2 まで在った単独の
    3つの輪の紙。2026-09-23 にシートへ一本化して取り除いた）は
    `readerStatsForSite(...).find((r) => r.scope === "work")` と書いていた。
    同じ読み方をする呼び出し元のために、前提は残して見張る。

...
const MARKDOWN_ALLOWED = new Set([
  ...
  */
  ...
  /*
    対話式プロット作成の依頼文（P-43）。**画面に出す文言は1つも持たない。**
    型によって差し込む段（枠の並び・次に埋める枠）が変わるので、依頼文を
    1つの複数行の文字列ではなく行の並びで組んでおり、「# 作品」のような
    見出しが1行の文字列として見える。中身はAIへ渡す材料の区切り
  */
  "src/prompts/plotDialogue.ts",
  // 記録ファイル（人が読む前提のログ）
  "src/core/logger.ts",

  ...
  describe(`${doc.name}の節番号`, () => {
    test("同じ番号の節が2つ無い", () => {
      const numbers = all
        ...
        // **節番号は「数字のうしろに区切りが来る」ものだけ。**
        //
        // 除きたいものを並べて潰すやり方では追いつかなかった（2026-09-21）。
        // 日付（`2026-08-05 …`）を `-` で除き、モデル名（`26b は…`）を英字で
        // 除いたが、次は**助数詞**（`3巡して分かった…`）が「節番号3」として
        // 通り、`## 3. 次にやること` と重複した。
        //
        // **並べる側を逆にする。** 節番号のうしろに来るのは
        // **空白（半角・全角）か `.`** だけである（`## 1. これまでの経緯`、
        // `#### 6.10.2 プロット逸脱…`）。それ以外が続くなら節番号ではない。
        //
        // **`.` を区切りに含めると、版番号を途中まで拾う。** `0.60.0（2026-09-13）…`
        // のような見出しで「0.60」までが節番号として通り、2つ並んで重複になった。
        // **番号のうしろは「（あれば）ピリオド1つ＋空白」でなければならない。**

...
describe("メニューから作る（手で書き写さない）", () => {
  ...
  test("相談の入口は、相談パネルのボタンだと書く", () => {
    // 0.29.24 で詳細メニューから隠した（作者の指定、2026-09-03）。
    // 隠した操作には入口を添える決まりだが、**この操作の主な入口は
    // 簡単ステップメニューではなく、横のパネルのボタン**である。
    // 「簡単ステップメニューから」とだけ書くと、いちばん近い道を隠す
    const line = MANUAL.split("\n").find((entry) =>
      entry.startsWith("- AI相談（大きく開く）")
    );

    expect(line, "マニュアルに項目が無い").toBeTruthy();
    expect(line).toContain("メインに表示");
    // 簡単ステップメニューにも残っているので、そちらも捨てない
    expect(line).toContain("簡単ステップメニュー");
  });

...
const ALLOWED = [
  ...
  /*
    **規則6の、ただ1つの例外**（作者の裁定、2026-09-13。CLAUDE.md に明記）。

    測った値の初期値を同梱する表で、鍵がモデル名そのものになる。
    規則の狙いは「**APIが教えてくれることを名前から当てにいくな**」で
    あって、ここが持つのは**APIがどこにも教えてくれない値**である
    （字/トークンの換算・実際に読めた長さ）。測るまで当て推量（0.7）で
    動くので、同梱しないと全員が同じ値を測り直すことになる。

    **薦めるモデルではない。** 名前が古くなっても、その行が引かれなく
    なるだけで、動きは当て推量へ落ちるだけ（悪くならない）。
  */
  "core/bundledTuning.ts",
  /*
    **同じ例外の、当たりの記録版**（作者の裁定 A3④、2026-09-23）。

    答え付きの台で測った「この機能で何件当てたか」で、鍵がモデル名になる。
    **勧める順には使わない**——順位は API のパラメータ数だけで決め、ここは
    確認画面へ「同梱の測定（日付）では 4/4」と添えるだけ（`core/largerModelAdvice.ts`）。
    名前が古くなっても、その行が引かれなくなって数字が添わないだけで済む。
    プロンプトの版が変わった行は使わない（古くなったら黙って使わない）。
  */
  "core/bundledFeatureAccuracy.ts",
  /*
    **同じ例外の、候補から外す記録版**（作者の裁定、2026-09-26 深夜）。

    誤字脱字検知の比べで当たり0件だったモデルを、選ぶ画面の一覧から外す表。
    鍵がモデル名になる。同梱の表と同じ約束で持つ——実際に測ったものだけ、
    何で測ったかと測った日を添える。**使えなくはしない**（名前を打てば
    選べ、割り当て済みならそのまま動く）ので、名前が古くなっても、
    その行が引かれなくなって一覧に戻るだけで済む。
  */
  "ai/hiddenModels.ts",
];

...
describe("人物抽出フロー", () => {
  ...
  */
  ...
  /*
    ...
    **人物を分けても、チャンクの鍵は変わらない**——鍵は本文の中身・
    プロバイダ・モデル・プロンプトの版だけで、人物の顔ぶれは入っていない。
    ここでは分けた直後の顔ぶれ（別人の印つきの元の人物と、中身が空の
    新しい人物）で抽出し直し、AIも疎通確認も呼ばないことを見る。
  */
  test("人物を分けた直後に抽出し直しても、キャッシュが効いてAIを呼ばない", async () => {
    state.cachedResults.set("chunk-1", {
    ...
    expect(state.generate).not.toHaveBeenCalled();
    expect(state.testConnection).not.toHaveBeenCalled();
    // 作者に見える言葉でも「AIは呼ばず」と言っている
    expect(
      showInformationMessage.mock.calls.map((call) => String(call[0])).join("\n")
    ).toContain("AIは呼ばず");
  });

...
describe("同期対象から外す規則", () => {
  ...
  test("既にある規則は重ねて足さない", () => {
    ...
    expect(missingIgnoreRules(existing)).toEqual([
      ...
      // 次の起動で追記される
      ".aiwriter/generated/",
      // 外部AIの利用許可の印（設計書6.87.10、0.64.3）。**同期すると、
      // リポジトリを共有した編集部の機械でも許可済みになる**
      ".aiwriter/external-access.json",
      // 外部AIからの「この項目を光らせて」（設計書6.104、0.75.6）。
      // **いまこの機械の画面で光らせてほしい、というその場限りの頼み**
      // なので同期しない（隣の external.jsonl は同期する）
      ".aiwriter/history/spotlight.jsonl",
      // 指示書の使い方（開いて使うか、開かずに使うか）の控え

...
/**
 * 同梱する書体が読めないとき（設計書6.65.11）。
 *
 * **書体は表紙・挿絵と違って、無くても本が困らない**（`serif` が受け止める）。
 * だから読めなくても本は出す。ただし黙らない——選んだつもりの書体が
 * 入っていないことに、作者が完了通知で気づけないといけない。
 */
describe("書体が読めないとき（設計書6.65.11）", () => {
  beforeEach(() => {
    put("本文/第1話.txt", "あ\n\nい");
  });

  test("本文用の書体のファイルが無ければ、本は出た上で理由を伝える", async () => {
    writeBook({
      title: "氷の街",
      fonts: { body: "素材/無い書体.ttf", heading: null },
    });
    ...
    expect(info).toContain(
      "本文用の書体「素材/無い書体.ttf」を読めませんでした。この書体は組み込んでいません。"
    );
  });
});

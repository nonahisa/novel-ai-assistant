...
describe("点検画面のHTML", () => {
  ...
  test("読みが無い名前は、見ていないと断る", () => {
    // もとは「読みが無いので見ていない名前」の1行だけを見ていた。この字は
    // 見出し（136行の<h2>。常に描かれる静的なHTML）にも入っているので、
    // 数える処理（renderUnreadable。0件とN件を出し分ける側）を消しても、
    // 見出しが残っている限り通ってしまう形だった。
    // script の中で、0件のときとN件のときの両方の文言を確かめる。
    const script = HTML.slice(HTML.indexOf("<script"), HTML.indexOf("</script>"));
    expect(script).toContain("読みが無いので見ていない名前（0件）");
    expect(script).toContain(
      "読みが無いので見ていない名前（' + list.length + '件）"
    );
  });
  ...
  test("何も書き換わらないことを画面に書く", () => {
    // もとは「更新」の1行だけを見ていた。「更新」は3か所にあり、うち
    // ［更新］ボタンのラベル（本題と無関係）だけが残っても通ってしまう形
    // だった。本題の説明文そのもの（読みを入れたあと、いつから点検対象に
    // なるか）を見る。
    expect(HTML).toContain(
      "設定資料パネルで読みを入れると、次の更新から点検の対象になります。"
    );
  });

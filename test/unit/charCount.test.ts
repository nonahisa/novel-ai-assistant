...
describe("文字数計測", () => {
  ...
  /**
   * **サロゲートペアの字は1字**（`countCodePoints` の見張り）。
   *
   * JavaScript の `String.length` は「𠮟」のような字を2と数える。
   * ここを `s.length` に戻しても、これまでの試験は全部緑のままだった
   * ——BMP内の字しか使っていなかったため、退行に気づけない。
   * 人名・地名に外字が入る作品で字数が水増しされるので、ここで留める。
   */
  test("サロゲートペアの字も1字として数える", () => {
    // 「𠮟る」は見た目2字。`String.length` では3になる
    expect("𠮟る".length).toBe(3);

    const counts = countChars("𠮟る");
    expect(counts.gross).toBe(2);
    expect(counts.net).toBe(2);
    // 原稿用紙のマスも見た目どおり2マス（20字で折り返すので1行）
    expect(counts.manuscriptLines).toBe(1);
  });

  /**
   * **全角スペースは純文字数に入れない**（`countChars` の空白除去の見張り）。
   *
   * JavaScript の `\s` は全角スペース（U+3000）を**含む**ので、
   * 文字クラスに全角スペースを重ねて書いてあるのは念のためである
   * （外しても結果は変わらない。2026-09-21に実際に外して確かめた）。
   * ここが留めるのは、空白の範囲を `[ \t]` のように狭めてしまう変更——
   * そうなると字下げの1字ぶんが純文字数へ紛れ込む。
   *
   * **原稿用紙の行数（`manuscriptLines`）とは扱いが違う。**
   * あちらは見た目の場所を数えるので、字下げの全角スペースは1マスを取る
   * （「字下げの全角スペースも1マスを使う」の試験）。純文字数は
   * 「書いた字の量」なので、空白は半角も全角も外す。
   */
  test("全角スペースは純文字数から外すが、総文字数には残す", () => {
    // 「　」＋「吾輩は猫である」の7字
    const counts = countChars("　吾輩は猫である");

    expect(counts.gross).toBe(8);
    expect(counts.net).toBe(7);
  });
});

 ...
 * 4. **許可の無い作品では断る**（ほかの道具とまったく同じ関門を通る）
 */
...
describe("申告が取れないときの落とし先", () => {
  ...
  it("ティアが取れなければ、ollama だけを軽量とみなす従来の判定へ落ちる", () => {
    expect(
      notice({ providerId: "ollama", tier: undefined }).profile
        .narrowContradictionCategories
    ).toBe(true);
    expect(
      notice({ providerId: "sakura", tier: undefined }).profile
        .narrowContradictionCategories
    ).toBe(false);
  });
});

...
describe("終わったときの知らせ", () => {
  ...
  test("前提が足りないだけのものは、「失敗」ではなく「飛ばした」と言う", async () => {
    /*
      **作者は「失敗」を見た時点で不具合を疑う**（実機確認 2026-09-06）。
      プロットの無い作品でまとめ実行を走らせると
      「プロット逸脱は失敗しました」と出ていた。壊れてはおらず、
      プロットを作れば走る——その違いが1行で伝わらなければならない。
    */
    selection = ["typos", "deviations"];
    remaining = { 誤字脱字: 1 };
    outcomes = {
    ...
    // **飛ばしても、残りは走らせる**（前提はこの検知だけのもの）
    expect(calls).toEqual([
      "novelai.checkTypos",
      "novelai.checkDeviations",
    ]);
    expect(announced).toEqual([
      "校正をまとめて実行しました。誤字脱字1件・" +
        "プロット逸脱は飛ばしました（プロットがまだ無いため）。" +
        "提案パネルで確認できます。" +
        "プロット逸脱は、「プロットをつくる」か「本文からプロットを起こす」で" +
        "プロットを作ってから実行してください。",
    ]);

  ...
  describe(`${doc.name}の節番号`, () => {
    test("同じ番号の節が2つ無い", () => {
      const numbers = all
        ...
        //
        // **モデル名も節番号ではない**（2026-09-21）。`26b は e4b より速い`
        // と `26b は同じ行を見て、逆の結論を出した` が並んだだけで、
        // 「節番号26が重複」と言い出した。**数字の直後に英字が続くものは
        // モデル名とみなす**（節番号のあとに来るのは空白と日本語である）。
        .map((h) => /^(\d+(?:\.\d+)*)(?![-\da-zA-Z])/.exec(h.text)?.[1])

...
| 役割 | 担当 | 具体的に |
|---|---|---|
| 全体計画・監視・**デバッグ** | **本体（Fable）** | 何を作るかの設計、作業の分割と順序、エージェントの結果の検収、**原因の分からない不具合の調査** |
| 実装 | **implementer（Opus）** | 固まった計画のコード化。新規実装・修正・リファクタリング |
| 軽いテスト | **light-tester（Sonnet）** | テストの実行と結果整理、期待値の更新、既存パターンを写した追加ケース |
...
- **版上げ・CHANGELOG・設計書・コミットは本体が行う**（エージェントには任せない。版の整合は `showVersion.test.ts` が見るが、采配は1か所に置く）

### 本体が自分でやってよいもの

数行の修正、設定値の変更、文書の編集、`grep`/`Read` による調査は、エージェントを経由するほうが高くつく。**分業は「まとまった実装」と「テストの往復」のためのもの**であり、儀式ではない。

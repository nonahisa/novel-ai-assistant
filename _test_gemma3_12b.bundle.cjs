...
var HONORIFIC_SUFFIXES = ["\u3055\u3093", "\u304F\u3093", "\u541B", "\u3061\u3083\u3093", "\u69D8", "\u3055\u307E", "\u6BBF", "\u5148\u8F29", "\u6C0F"];
function normalizeName(s) {
  const base = s.replace(/[\s　・･]/g, "").replace(/[０-９]/g, (c) => String.fromCharCode(c.charCodeAt(0) - 65248)).toLowerCase();
...
// src/features/extractCharacters.ts
var INVALID_NAME_PATTERN = /^(null|undefined|不明|なし|n\/?a|none|[（(]?主[）)]?|主人公)$/i;
...
=== ${MODEL} \u7D50\u679C\u30B5\u30DE\u30EA ===`);
  ...
  console.log(`\u6240\u8981\u6642\u9593\u5408\u8A08: ${(totalMs / 1e3).toFixed(1)}\u79D2 / \u5E73\u5747: ${Math.round(totalMs / timings.length)}ms`);
  console.log("\n\u4EBA\u7269\u540D\u4E00\u89A7\uFF08role / aliases\u4ED8\u304D\uFF09:");
  for (const c of merged.characters) {
    console.log(`  - ${c.name}  role=${c.role ?? "null"}  aliases=[${c.aliases.join(", ")}]`);
  }
  const outFile = path2.join(WORK_DIR, ".aiwriter", "compare_gemma3_12b.json");

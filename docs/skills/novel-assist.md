...
能力・組織・場所・世界観も、`recordKind`（`ability`／`organization`／`location`／`world`）を渡せば同じ道具で置ける。**台帳にある記録の更新案だけ**で、新しく作る案は断られる（作るときは作者に頼む）。名前で引き当てる（別名では引き当てない）。受け付けない欄（作者の覚え書き・名前・ID など）は置かれずに `skipped` で返るので、落ちた欄と理由を作者に伝える。作者が確定させた記録には置けない。

...
const DENIAL_PATTERN =
  /((矛盾|食い違い?)(で)?は?(あり)?(し)?(て)?(い)?(ない|ませ|なく)|一致してい(る|ます)|問題(は)?(あり)?ませ|整合(性)?(は|が)?(取|と)れて(いる|います)|整合してい(る|ます)|齟齬(は)?(あり)?(ませ|ない)|破綻(は)?(し)?(て)?(い)?(ない|ませ))/;
...
const HEDGED_DENIAL_PATTERN =
  /(矛盾|食い違い?)と(まで)?は?((言|い)(え|い切れ)|断定でき|判断でき)(ない|ませ|ず|なく)/;
...
const UNSURE_DENIAL_PATTERN =
  /(矛盾|食い違[いっ])(し?て)?(いる|います)?(のか|か)(どうか)?(は|が)?(不明|判断でき(ない|ませ|ず)|(分|わ)か(ら|り)(ない|ませ|ず)|定かで(は)?(ない|ありませ))/;
...
const AFFIRMATION_PATTERN =
  /(矛盾してい(る|ます|た)(?![かの])|矛盾する(?![かの])|食い違ってい(る|ます|た)(?![かの]))/;

 ...
 * ### 分かっている重複（**まだ直していない**）
 *
 * 全数調査で見つかった、同じ意味の値が複数箇所にあるもの。**どれも
 * 挙動を変えずには寄せられないか、寄せる利益が小さい**ので第1段では
 * 触っていない。新しく書くときに増やさないための覚え書きである。
 *
 * - AI応答を失敗ログへ載せるときの切り詰め字数が、`features` の13ファイルに
 *   独立して書かれている（値も意図も同一）。**いちばん大きい重複**
 * - 引用抜粋の字数が `core/deviationValidation.ts` と
 *   `core/episodePlotValidation.ts` に同名同値で二重定義
 * - `OPENING_EXCERPT_CHARS` が `generateBlurb.ts` と `generatePlot.ts` で
 *   **同名・別値**。意図は別だが、名前が同じなので取り違えやすい
 * - ログのバイト上限が `logger.ts` / `chatLog.ts` / `usageLog.ts` の3つに
 *   あり、値も割れている
 *

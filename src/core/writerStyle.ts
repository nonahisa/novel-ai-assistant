/**
 * 作家タイプ診断——執筆スタイルと、はじめの案内（設計書6.90）。
 *
...
export type TutorialGoal =
  ...
  | "publish"
  /**
   * 手が止まっている。書き進めたい（作者の指摘、2026-09-13
   * 「スランプを脱出したい作家に対する支援を選択肢から感じられませんでした」）。
   *
   * **病名のように書かない。** 「スランプ」と札を貼ると、作者は
   * それを自分の状態の診断として受け取る（6.86.2 で、自信度を本人に
   * 見せないと決めたのと同じ理由）。並べるのは**したいこと**である。
   */
  | "unstick"
  /** 人の作品を見る側で使いたい */
  | "review";
...
export interface TutorialAdvice {
  ...
  later: string[];
  /**
   * **できるけれど、いまは出さないもの**（0.52.0）。
   *
   * `later`（まだできないこと）と混ぜない。混ぜると「執筆統計は使えません」
   * と読めてしまい、**事実でないことを伝える**ことになる。
   * 出さないと決めたのなら、**出さない理由まで書く**。
   */
  withheld?: string[];
}
...
export function tutorialGoals(
  ...
  if (!hasWork) {
    ...
    if (style.situation === "starting") {
      ...
      // **書き出せない、も行き先にする。** 始め方が分かっていても
      // 手が動かないことはある
      goals.push({
        goal: "unstick",
        label: "書き出せない。取っ掛かりがほしい",
        why: "まだ書き始めていなくても、手が止まることがあるためです",
      });
    }
  } else {
    goals.push({
    ...
    // **手が止まっている人の行き先を、必ず1つ置く**（作者の指摘、2026-09-13）。
    // 「続きを書く」の隣に置くのは、そこを選んで戻ってくる人がいるからである
    goals.push({
      goal: "unstick",
      label: "手が止まっている。書き進めたい",
      why:
        "書き進める手掛かりが要るときに、いつでも選べるようにしてあるためです",
    });
    goals.push({
      goal: "polish",
      label: "書いたものを直す",
...
/**
 * 手が止まっているとき（作者の指摘、2026-09-13）。
 *
 * **新しく決めさせない。** 書けないときに「プロットを立て直しましょう」と
 * 言うのは、いちばん重い仕事を勧めることになる。ここで並べるのは
 * **いま在るものを見返す口**である——前の話の終わり、張ったままの伏線、
 * 書きながら置いた付箋。材料はもう作品の中にある。
 *
 * **執筆統計は勧めない。** 書けていない日が並ぶ画面なので、
 * いま開いても手掛かりにならない（`later` にそう書いて、押せる形では出さない）。
 *
 * **品定めをしない。** 「スランプ」「行き詰まっている」とこちらから言わない
...
function unstickAdvice(style: WriterStyle): TutorialAdvice {
  ...
  // まだ作品が無いなら、見返す材料もない。始め方の口だけを軽く出す
  if (style.situation === "starting") {
    return {
      advice:
        "書き出せないときは、決める量を減らすのが早いことがあります。" +
        "作品ぜんぶの筋を決めなくても、1話ぶん——いや1場面ぶんでも書き始められます。" +
        "先に相談していただいても構いません。ここで答えたことは、そのまま材料になります。",
      steps: [
        talk,
        {
          command: "novelai.createWorkFromManuscript",
          label: "本文から書き始める",
          why:
            "本文/001.txt を作るだけです。書けたところまでで構いません" +
            "——あとからプロットへも移れます",
        },
      ],
      later: [
        "書き始めたあとは、この案内に「手が止まっている」がもう一度出ます",
      ],
    };
  }
  ...
  if (style.plan === "improviser") {
    ...
    steps.push({
      ...
      why:
        `${WRITER_PLAN_TYPES[style.plan].label}と答えていただいたためです。` +
        "作品ぜんぶではなく、次の1話ぶんだけ決めれば書き出せることがあります",
    });
  }

  return {
    advice:
      "書けない日は、書くより読むほうが早く戻れることがあります。" +
      "手が止まっているときに効くのは、新しく決めることより、いま在るものを見返すことです。" +
      "前の話がどこで終わったか、張ったままの伏線、書きながら置いた付箋" +
      "——材料はもう作品の中にあります。上から順にご覧ください。",
    steps,
    later: [],
    withheld: [
      "執筆統計。書けていない日が並ぶ画面なので、手が動き出してから見るほうがよいと思います（「執筆データ」からいつでも開けます）",
    ],
  };
}

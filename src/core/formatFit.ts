...
export function formatWarningFor(
  ...
  if (format === "shortCollection") {
    return {
      message: "短編集では、本文からプロットを逆算する前提が合いません。",
      detail:
  ...
  if (format === "sns") {
    return {
      message: "SNS記事では、本文からプロットを逆算する前提が合いません。",
      detail:
        "投稿は連続した物語ではないため、あらすじを並べても筋になりません。" +
        "投稿どうしを繋いだ、実際には無い話が出てきます。",
    };
  }
  // **創作メモ集は、ステップと右クリックからは既に消えている**（設計書6.70.1）。
  // それでも断りを残すのは、詳細メニューには全機能を置いたままだからである
  if (format === "memo") {
    return {
      message: "創作メモ集では、本文からプロットを逆算する前提が合いません。",
      detail:
        "メモは1本の物語ではないため、並べても筋になりません。" +
        "書き溜めたメモを繋いだ、実際には無い話が出てきます。",
    };
  }
  return undefined;
}

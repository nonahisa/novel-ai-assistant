...
export type CollectionScan =
  ...
  /** 指定されたフォルダー自体が作品だった（書庫ではない） */
  | { kind: "single_work" }
  /**
   * 自分も作品に見えるが、**中にも作品が並んでいる。**
   *
   * **機械には決められない。** 書庫の直下に `.aiwriter/config.json` が
   * 残っていることがある（書庫の仕組みができる前に、その全体を1作品として
 ...
 * 直下に話数として読めるファイルがあるか。
 *
 * **書庫そのものを作品と誤認しない。** 書庫の直下にも `README.md` や
...
export async function scanCollection(
  ...
  // **自分が作品に見えても、中を見る。**
  // 以前はここで打ち切っていたが、書庫の直下に設定ファイルが残っている
  ...
  switch (scan.kind) {
    case "single_work":
      return (
        ...
        "書庫として登録するのではなく、「作品を追加」から1作品として登録してください。"
      );
    case "no_works":
      return (
        `「${name}」の中に作品が見つかりませんでした。` +
        "書庫は、作品のフォルダーを並べた形を想定しています" +

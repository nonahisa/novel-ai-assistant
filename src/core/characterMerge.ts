...
export interface MergeResult {
  ...
  /**
   * 以前の形（性格を「作中の変化」や「食い違い」として積んでいた形）から、
   * 性格を面ごとに積む形へ移した人物の名前（作者の裁定、2026-09-24 夜）。
   *
   * **黙って書き換えたことにしない。** 本体の性格が「A／B」に変わり、
   * 年表から性格の変化が消えるので、件数を完了報告に出す。
   */
...
export function mergeExtractedCharacters(
  ...
  for (const item of extracted) {
    ...
    // **面を足す前に、古い形の性格を面へ移す**（2026-09-24 夜）。
    // 先に面を足すと「面がある」ことになり、以前に「変化」として積まれた
    // 性格が移されないまま年表に残る
    const migrated = migratePersonalityInPlace(match, personalityMigrated);
  ...
  for (let index = 0; index < result.length; index++) {
    ...
    // 以前の形で積まれた性格を面へ移す（2026-09-24 夜）。畳むのと同じく
    // **今回触れなかった人物にも掛ける**——年表に残った起きていない
    // 「性格の変化」を消すのが目的なので、今回の更新分に絞ると残り続ける
    if (migratePersonalityInPlace(character, personalityMigrated)) {
      changedIds.add(character.id);
      if (!updated.includes(character.name)) updated.push(character.name);
    }
  ...
  return {
    ...
    personalityMigrated: result
      .filter((character) => personalityMigrated.has(character.id))
...
/**
 * 古い形の性格を面の形へ移し、**渡したレコードへ書き戻す**（マージの中は
 * レコードを直に触る作法なので揃える）。移したら true。
 *
 * 作者が確定させた人物には掛けない（CLAUDE.md 規則2）。
 */
function migratePersonalityInPlace(
  target: Character,
  migratedIds: Set<string>
): boolean {
  if (!target.autoGenerated) return false;
  const migrated = migratePersonalityFacets(target);
  if (!migrated) return false;
  target.personality = migrated.personality;
  target.personalityFacets = migrated.personalityFacets;
  target.changes = migrated.changes;
  target.conflicts = migrated.conflicts;
  migratedIds.add(target.id);
  return true;
}

/**
 * 抽出した性格を、面として足す（作者の裁定、2026-09-24 夜）。
 *
 * **性格だけは `fillOrConflict` を通さない。** あちらは「今の値が1つ」の
 * 項目の規則で、違う話の値は作中の変化として本体を入れ替え、同じ話の
 * 違う値は食い違いとして止める。性格は同時に成り立つ面を重ねて描かれる
 * ので、どちらも誤りになる（`personalityFacets.ts`）。
 */
function addExtractedPersonality(
  target: Character,
  ...
  // 「（本文から読み取れる記述なし）」は空欄と同じ（`fillOrConflict` と同じ）
  if (!isMeaningfulValue(value)) return false;
  return addPersonalityFacet(target, value, chapters, evidence);
}
...
function applyExtracted(
  ...
  // 性格は面として積む（上書き型の項目と規則が違う。`addExtractedPersonality`）
  changed =
...
export function insertFieldValue(
  ...
  let changed: boolean;
  // 性格は面として足す（抽出のマージと同じ規則。食い違いにはならない）。
  // 面は直に書き換えるので、ここでも要素まで写す。
  //
  // **作者が確定させた人物は、これまでどおり食い違いとして作者へ回す。**
  // 面として足すと、作者の書いた本体の文章へこちらが言葉をつなぐことになる
  // （CLAUDE.md 規則2）。空欄なら埋まるのは同じ
  if (field === "personality" && character.autoGenerated) {
    target.personalityFacets = (character.personalityFacets ?? []).map(
      (facet) => ({ ...facet, chapters: [...facet.chapters] })
    );
    // 古い形の性格は、面を足す前に面へ移す（マージと同じ順）
    const migrated = migratePersonalityInPlace(target, new Set());
    const added = addExtractedPersonality(target, value, chapters, null);
    changed = migrated || added;
  } else {
    changed = fillOrConflict(target, field, value, chapters, reported);
  }

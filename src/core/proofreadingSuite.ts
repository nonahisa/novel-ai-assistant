...
export interface SuiteFeatureContext {
  ...
  /**
   * **実行の札（設計書6.76）は、まとめ実行が丸ごと持っている。**
   *
   * 作者の報告（2026-09-13）：「校正をまとめて実行と、資料生成の
   * まとめて抽出を時間差で実行したところ、途中で差し込まれたように
   * 見えます」。
   *
   * それまでは、まとめ実行が札を取らずに**各機能へ順に取らせていた**。
   * 機能と機能のあいだで札がいったん空くので、あとから押した
   * 「まとめて抽出」がそこへ入り、7工程のまとめ実行が3工程目で
   * 止まって待つことになった。**作者は時間差で押したのだから、
   * あとのものは後ろに並ぶはずである。**
   *
   * 札を持ったまま各機能を呼ぶので、機能側はもう取らない（取ると
   * 自分の持つ札を自分で待つ形になり、永久に進まない）。
   */
  readonly holdsRun?: true;
  /**
   * 前提が無くて走れなかった理由を伝える口。
   *
...
export interface SuiteAwareOptions {
  ...
  /**
   * **実行の札は、まとめ実行が持っている。** 自分で取らないこと。
   *
   * 取ると、自分の持つ札を自分で待つ形になり、永久に進まない
   * （`aiTurn.ts` の `alreadyHeld`）。
   */
  suiteHoldsRun?: boolean;
  /**
...
/**
 * まとめ実行が実行の札を持っているか（コマンドの第2引数を読む）。
 *
 * **確認済みかどうかとは別に見る。** まとめ実行以外から
 * `suiteConfirmed` 相当の呼ばれ方をする道が増えたときに、
 * 札まで「持っているつもり」になると誰も取らないまま流れてしまう。
 */
export function isSuiteHoldingRun(options: unknown): boolean {
  return suiteContextOf(options)?.holdsRun === true;
}

...
function manualHeader(): string {
  return [
    ...
    "",
    // 場面別案内への入口（作者の依頼、2026-09-23）。この文書は一覧なので、
    // 「どの順で押すか」を画面で示す案内はそちらへ渡す
    "新作の始め方や推敲・投稿の準備など、場面ごとの手順を画面で順に示してほしいときは、「ヘルプ → 場面別案内」から場面を選んでください。",
    "",
...
function actionChapter(allowsProcesses: boolean): string {
  const lines: string[] = [
    ...
    "一部の操作はメニューに出さず、原稿エディター・設定管理・簡単ステップメニューなどから使います（その旨を添えてあります）。",
    "",
  ];
 ...
 *
 * 落とすのは画面用の強調の印——メニューのホバーは `MarkdownString` なので
 * 印が効くが、こちらは本文の中に混ざると読みにくいだけである。記号そのものを
 * 文字列に書かない（画面に出す文字を見張る試験に引っかかる）。
 *
 * **改行も畳む**（0.75.7。説明文を二段組みに書き換えたとき）。マニュアルは
 * 「- 名前: 説明」の一覧で、説明は1行に収まる前提である。改行をそのまま
 * 流すと2行目以降が箇条書きの外へこぼれ、**そこで一覧が切れる**。
 * 事柄は「・」で始まるので、畳んでも切れ目は読み取れる。
 */

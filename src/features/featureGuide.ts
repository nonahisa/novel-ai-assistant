...
- 設定管理（詳細メニュー →「統合小説執筆環境設定」→「設定管理」）: 文字数の数え方や待ち時間などの設定。セットアップ・ベクトル検索準備・機能別AI割当・確認省略解除は、該当する設定の説明の下のリンクから実行する。作者／編集者切替は詳細メニューの「作品管理」→「編集部連携」にもある
- 詳細メニュー（左）: 下に並ぶ操作の一覧。分類→小分類→操作の3階層
- AIに相談（左）: この画面。開いているファイルについて日本語で相談できる
- 設定資料（エディター領域）: 登場人物・能力・組織・場所・世界観を見て、書き換えたり、AIに相談したりする画面
- 提案（エディターの右の列）: 誤字脱字・表記ゆれの指摘が並ぶ。1件ずつ「適用」「無視」を選ぶ
...
- ヘルパーへ渡す形でコピー（新話投稿）: 変換済みの本文を、相棒のブラウザ拡張「統合小説執筆環境ヘルパー」が投稿画面の欄へ流し込める形でコピーする。「新話投稿」の案内の中から選ぶ。カクヨムとアルファポリスにしか出ません。送信は必ず作者が押します
- Xへ貼り付ける（更新告知文）: 告知文と作品一覧のURLを入れた状態で、Xの投稿画面を開く。「更新SNS告知文作成」の結果の画面にあるボタン。投稿ボタンは作者が押します
...
- 普通のエディターでは「口述文整形」（コマンドパレット）を使います。整える範囲は選んだところです
- 整えたあとは Ctrl+Z で元に戻せます。整えている間に本文が変わっていたときは、置き換えずに知らせます
- 音声認識そのもの（whisperなど）を拡張機能に載せることは、いまはしていません

【noteへ貼る】
- 「投稿用変換・コピー」で貼り付け先に note を選ぶと、noteのエディタが読める形に整えてコピーします。noteの本文欄へそのまま貼れば、見出し・太字・取り消し線・リンク・区切り線・箇条書き・番号付き・コードブロック・引用として出ます
...
const GUIDE_BUNDLE_SPLITS: Record<string, readonly GuideBundleSplit[]> = {
  ...
  */
  "校正・校閲": [
    {
      // 受け皿。期限切れ指摘消去など、表に載せていない操作も残る
      // （校閲ロック・編集部の提案は 2026-09-23 に作品管理の「編集部連携」へ移った）
      label: "校正",
      commands: [
        ...
        // 冒頭診断は 2026-09-23 に「読者診断」の小分類へ移ったので、ここには無い
      ],
    },
    {
      label: "伏線・矛盾",
 ...
 * 残すのは2種類だけである。
 *
 * 1. **1行目の1文目**——この作品の説明文は「何ができるか」を先頭に書く決まりで
 *    揃えてある
 * 2. **しないことの断り**——「原稿は書き換えない」「元のフォルダーは消さない」
 *    「AIは呼ばない」。**作者がいちばん知りたいこと**であり、落とすとAIが
 *    「する」と答えかねない
 *
 * **行で見る**（0.75.7。説明文を二段組みに書き換えたとき）。説明文は
 * 「要約の1行 → 空行 → 『・』の箇条書き → 空行 → 断りの1行」という形に
 * 揃えてあり、**箇条書きには句点が無い**。文で割ると箇条書きごと1文に
 * なってしまい、改行を含んだ塊が相談の一覧（1操作＝1行）へ流れ込む。
 *
 * 断りは語尾では見分けない。二段組みへ書き換えたときに文体が「〜ません」から
 * 言い切りへ変わったので、語尾で拾うと**全部の断りが落ちる**。
 * 形のほうで見分ける——箇条書き以外の行は断りしかない。
 */
export function shorten(text: string): string {
  ...
  // 1行目が複数の文でできている（一段のままの説明文）ときは、その1文目だけ。
  // **要約は体言止めで句点を持たない**ので、断りとつなぐ前にここで補う
  // （補わないと「話ごとの出来事AIは呼ばない。」と続いて読めなくなる）
  const head = splitSentences(lines[0])[0] ?? lines[0];
  const kept = [head.endsWith("。") ? head : `${head}。`];
  for (const line of lines.slice(1)) {
    // 箇条書き（「・」で始まる）は但し書きなので落とす。
    // **残る行は断りの1行しかない**——二段組みの形がそう決まっている

...
- 設定管理（詳細メニュー →「拡張機能の設定」→「設定管理」）: 文字数の数え方や待ち時間などの設定。セットアップ・意味検索の準備・機能ごとのAI割り当て・作者と編集者の切り替えは、該当する設定の説明の下のリンクから実行する
...
- ヘルパーへ渡す形でコピー（新話を投稿）: 変換済みの本文を、相棒のブラウザ拡張「統合小説執筆環境ヘルパー」が投稿画面の欄へ流し込める形でコピーする。「新話を投稿」の案内の中から選ぶ。カクヨムとアルファポリスにしか出ません。送信は必ず作者が押します
...
- 普通のエディターでは「口述した文を整える」（詳細メニュー → 原稿づくり）を使います。整える範囲は選んだところです

...
function collectValidationFixes(
  ...
  target.droppedSharedFamilyNameAliases.push(
...
export async function extractCharacters(
  ...
  if (pending.length === 0) {
    ...
    */
    ...
    const costNotice = buildExtractionCostNotice(
      ...
      pending,
      // 目安は「実際に送るもの」で測る。裏付け用の全件ではなく、
      // プロンプトへ渡すぶん（人数で切ったもの）を渡す
      buildKnownCharacterNamesForPrompt(loaded.characters, []),
  ...
  */
  ...
  /**
   * 作者が手で書いたレコードの名前。**その人の別名はコードでも触らない**
   * （CLAUDE.md 規則2）。姓が共有されていても、作者がそう書いたのなら
   * AIの読みを直す話ではない
   */
  const authorEditedNames = loaded.characters
    .filter((character) => character.autoGenerated === false)
  ...
  /**
   * 既存の能力・場所・組織の名前と別名。AIに「既知」として見せているので、
   * その名前で返ってきたものは、その話の本文に名前が無くても根拠なしに
   * しない（2026-09-24 の裁定。引用の照合は残る）。この回で受け入れた分は
   * 集める側が自分で足す
   */
  const existingSettingNames = {
    abilities: existingAbilities.flatMap((item) => [item.name, ...item.aliases]),
    locations: existingLocations.flatMap((item) => [item.name, ...item.aliases]),
    organizations: existingOrganizations.flatMap((item) => [
      item.name,
      ...item.aliases,
    ]),
  };
  ...
  await withAiTurnProgress(
    ...
    async (progress, token) => {
      ...
      for (let position = 0; position < queue.length; position++) {
        ...
        if (cached) {
          ...
          const validated = validateCharacterExtractResult(
            ...
            chunk,
            {
              ...
              authorEditedNames,
              knownRecordNames: buildKnownCharacterNames(
                loaded.characters,
                extractedAll
              ),
            }
          ...
          settings.collect(cachedResult, chunk, existingSettingNames);
          done++;
          continue;
        }
        ...
        // **上限は人数で数える**（名前の件数で切ると、別名の多い作品で
        // 後ろの人物が丸ごと落ちる。`buildKnownCharacterNamesForPrompt`）
        const knownNames = buildKnownCharacterNamesForPrompt(
          loaded.characters,
          extractedAll
        );

        const known = {
          characters: knownNames,
        ...
        try {
          ...
          if (!parsed) {
            ...
            const validated = validateCharacterExtractResult(parsed, chunk, {
              knownNames: existingCharacterNames,
              authorEditedNames,
              // AIに「既知の登場人物」として見せた顔ぶれ（既存＋この回の分）。
              // その名前で返った人物は、本文に名前が無くても根拠なしにしない
              knownRecordNames: buildKnownCharacterNames(
                loaded.characters,
                extractedAll
              ),
            ...
            rejectedCandidates.push(...validated.rejected);
            collectValidationFixes(validationFixes, validated);
            settings.collect(parsed, chunk, existingSettingNames);
...
function describeValidationFixes(fixes: ValidationFixCounts): string {
  ...
  if (fixes.droppedSharedFamilyNameAliases.length > 0) {
    lines.push(
      `2人以上が名乗る姓（苗字）だった別名を ${
        fixes.droppedSharedFamilyNameAliases.length
      }件 外しました（${describeDroppedAliases(
        fixes.droppedSharedFamilyNameAliases
      )}）`
    );
  }
...
export function buildExtractionCostNotice(
  ...
  // **ここでは切らない。** 渡ってくるのは送るぶんそのもの
  // （`buildKnownCharacterNamesForPrompt` が人数で切った一覧）なので、
  // 重ねて切ると実際より少ない目安を出してしまう
...
/*
  **既知の人物名の組み立ては `core/knownCharacterNames.ts` にある**
  （0.64.3で移した）。ここから再輸出するので、**使う側の既定は今までどおり**。

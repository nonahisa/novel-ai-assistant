...
interface DetailField {
  ...
  hint?: string;
  /**
   * 退けた関係（作者の裁定、2026-09-23）。**関係欄だけが持つ。**
   *
   * 1件ずつ「退けた記録から外す」ボタンを添えて並べる。外すのは記録だけで、
   * 関係そのものは戻さない（戻すかどうかは作者が関係欄で決める）。
   */
  rejected?: Array<{ target: string; relation: string; note: string }>;
}
...
interface DetailView {
  ...
  reference: Array<{
    ...
    value: string;
    /**
     * その行に添える操作（設計書6.18）。3種類ある。
     * - `promoteConflict`：食い違いを「作中の変化」として確定させる
     * - `dropChanges`：記録された変化から、誤って入ったものを落とす
     * - `confirmChanges`：根拠が無いので本体へ入れていない変化（要確認）を、
     *   作者が正しいと認める（作者の裁定、2026-09-23）
     ...
     *
     * - `dropConflictValue`：食い違いの値の1つを「こちらは誤り」として一度で
     *   落とす。値ごとに札を分けるのは、押した先で選ばせると2手に戻るため
     *   ——食い違いの値はふつう2〜3個で、変化の記録（何件も積もる）とは数が違う
     * - `undoRejectedValue`：「誤りとして落とした値」の印を1件外す（2026-09-26）。
     *   食い違いの記録も本体の値も戻さない
     */
    valueActions?: Array<{
      label: string;
      ...
      field: string;
      value: string;
      kind: "dropConflictValue" | "undoRejectedValue";
    }>;
  }>;
  /**
...
export class SettingsPanel {
   ...
   * 「閲覧がわかりにくい」と言われていた。
   *
   * **読むだけにする。** 紹介文は「作品紹介文」、あらすじは
   * 「各話あらすじ」が真実の在り処を持っている。ここで書き換えられると、
   * どちらが正しいのか分からなくなる。
   */
  ...
  private detailOf(kind: SettingsKind, id: string): DetailView | undefined {
    ...
    if (kind === "character") {
      ...
      return {
        ...
        reference: [
          // 性格の面（2026-09-24 夜）。どの話で見えた面かと、作中で変わった
        ...
        fields: [
          ...
          // 相手側の記録は説明に並べるだけで、こちらからは書き換えない
          {
            ...
            rejected: character.rejectedRelations.map((entry) => ({
              target: entry.target,
              relation: entry.relation,
              note: describeRejectedRelation(entry),
            })),
          },
    ...
    try {
      switch (message.type) {
        ...
        case "deleteNote":
          ...
          return;
        case "forgetRejectedRelation":
          await this.handleForgetRejectedRelation(
            message.id,
            message.target,
            message.relation
          );
        ...
        case "dropConflictValue":
          ...
          return;
        case "undoRejectedValue":
          await this.handleUndoRejectedValue(
            message.id,
            message.field,
            message.value
          );
        ...
        case "confirmChanges":
          ...
          return;
        case "markPersonalityChange":
          await this.handleMarkPersonalityChange(message.id);
          return;
  ...
  private async handlePromoteConflict(
    ...
    // **作者が押したので、作者が認めた変化として記録する**（2026-09-23）。
    // 根拠の無い値でも、以後の抽出で本体を動かせる変化になる
  ...
  /**
   * 「誤りとして落とした値」の印を1件外す（作者の裁定、2026-09-26）。
   *
   * 外すのは「次の抽出や外部AIの提案が、この値をまた出してよい」と許すことで、
   * **食い違いの記録も本体の値も元へ戻さない**（`undoRejectedValue`）。落とした
   * ときに本体が入れ替わっていても、それは作者が決めた結果なので黙って巻き
   * 戻さない。確認の窓は出さない——外しても値は資料に入らず、次の抽出で
   * また出れば食い違いとして並ぶだけなので、取り違えても失うものが無い。
   *
   * 保存は人物の既存の経路（`persist` → `saveOrUpdate`。読み込み時のハッシュと
   * 照合し、外で書き換えられていれば止まる）。`autoGenerated` は変えない
   * （退けた関係を外すときと同じ。作者が触ったのは記録の1件である）。
   * 落とした値は資料集（`characters.md`）に載せていないので、作り直さない。
   */
  private async handleUndoRejectedValue(
    id: string,
    field: string,
    value: string
  ): Promise<void> {
    ...
    const updated = undoRejectedValue(character, field, value);
    if (!updated) {
      // 別の窓で先に外された場合など。読み直して今の状態を見せる
      await this.reloadAfterSave(
        "character",
        id,
        `「${field}」の「${value}」は、誤りとして落とした値にもうありません。`
      );
      return;
    }
    ...
    await this.reloadAfterSave(
      "character",
      id,
      `「${field}」の「${value}」の誤りの印を外しました。` +
        "次の抽出でこの値がまた出れば、食い違いとして並びます。"
    );
  }
   ...
   *
   * 根拠（本文の引用）の無い変化は、抽出が記録しても本体の値を動かさない。
   * 作者が画面で認めたら、その変化に印（`confirmed`）を立て、後ろの話の値なら
   * 本体へ入れる。**値はどれも消さない。** 誤りだったなら、隣の
   * 「誤りを落とす」で作者が落とす。
   *
   * `autoGenerated` は変えない（`handlePromoteConflict` と同じ理由）。
   */
  private async handleConfirmChanges(id: string, field: string): Promise<void> {
    const character = this.characters.find((entry) => entry.id === id);
    if (!character) {
      this.post({ type: "error", message: "選択した設定が見つかりません。" });
      return;
    ...
    const outcome = confirmHeldChanges(character, field);
    if (outcome.confirmed === 0) {
      // 別の窓で先に処理された場合など。読み直して今の状態を見せる
      await this.reloadAfterSave(
        "character",
        id,
        `「${field}」に、要確認の変化はありません。`
      );
      return;
    }
    ...
    await this.reloadAfterSave(
      "character",
      id,
      `「${field}」の変化を${outcome.confirmed}件、正しいものとして認めました。`
    );
  }

  /**
   * 積んだ性格の面のうち、作中で変わったものを作者が決める（2026-09-24 夜）。
   *
   * 性格は既定では面として積む（`personalityFacets.ts`）。「臆病だったのが
   * 勇敢になった」のような本当の変化は本文からコードで見分けられないので、
   * ここで作者に選ばせる。変わる前の面は過去の面になって本体から外れ、
   * 両方が作者の認めた変化として記録される。**値はどれも消えない。**
   *
   * `autoGenerated` は変えない（`handlePromoteConflict` と同じ理由）。
   */
  private async handleMarkPersonalityChange(id: string): Promise<void> {
    const character = this.characters.find((entry) => entry.id === id);
    if (!character) {
      this.post({ type: "error", message: "選択した設定が見つかりません。" });
      return;
    }
    const active = activeFacets(character.personalityFacets ?? []);
    if (active.length < 2) {
      // 別の窓で先に処理された場合など。読み直して今の状態を見せる
      await this.reloadAfterSave(
        "character",
        id,
        "性格の面が2つ以上ないので、変化にできる組がありません。"
      );
      return;
    }

    const before = await vscode.window.showQuickPick(
      [...personalityFacetPicks(active), cancelItem()],
      {
        title: "性格：作中で変わる前の面を選ぶ（1/2）",
        placeHolder: "この面は過去の面になり、性格の欄から外れます（記録には残ります）",
        ignoreFocusOut: true,
      }
    );
    // 「取りやめる」や Esc なら何もしない
    if (!before || !("value" in before)) return;
    const after = await vscode.window.showQuickPick(
      [
        ...personalityFacetPicks(
          active.filter((facet) => facet.value !== before.value)
        ),
        cancelItem(),
      ],
      {
        title: `性格：「${before.value}」から変わった先の面を選ぶ（2/2）`,
        placeHolder: "作中でこの面へ変わった、として記録します",
        ignoreFocusOut: true,
      }
    );
    if (!after || !("value" in after)) return;

    const outcome = markPersonalityChange(character, before.value, after.value);
    if (!outcome) {
      await this.reloadAfterSave(
        "character",
        id,
        "その面は、すでに変化として記録されています。"
      );
      return;
    }
    await this.persist("character", outcome.character);
    await this.reloadAfterSave(
      "character",
      id,
      `性格の「${before.value}」を、「${after.value}」へ作中で変わった面として記録しました。` +
        (outcome.bodyUpdated
          ? ""
          : "性格の欄は手で書き換えられていたので触っていません。必要なら欄を直してください。")
    );
  }
   ...
   *
   * **変化の記録は消さない。** 要らなければ「変化（personality）」の行の
   * 「誤りを落とす」で落とす（値を消す操作は1か所にしか置かない）。
   */
  private async handleRestorePersonalityFacet(id: string): Promise<void> {
    const character = this.characters.find((entry) => entry.id === id);
    if (!character) {
      this.post({ type: "error", message: "選択した設定が見つかりません。" });
      return;
    }
    const past = (character.personalityFacets ?? []).filter(
      (facet) => facet.supersededBy
    );
    if (past.length === 0) {
      await this.reloadAfterSave("character", id, "過去の性格の面はありません。");
      return;
    }
    const chosen = await vscode.window.showQuickPick(
      [...personalityFacetPicks(past), cancelItem()],
      {
        title: "性格：いまも成り立つ面へ戻す",
        placeHolder: "戻した面は、性格の欄の末尾へつながります",
        ignoreFocusOut: true,
      }
    );
    if (!chosen || !("value" in chosen)) return;

    const outcome = restorePersonalityFacet(character, chosen.value);
    if (!outcome) {
      await this.reloadAfterSave(
        "character",
        id,
  ...
  private applyEdits(
    ...
    if (kind === "character") {
      // 顔ぶれを渡すのは、関係欄で消した関係を「退けた関係」として残すとき、
      // 相手を相関図と同じ規則で引き当てるため（`rejectedRelations.ts`）
  ...
  private async handleEnrich(
    ...
    if (speechEchoes > 0) {
      useLogFile(this.work.folderPath);
      logFailure("再読込の口調の提案のうち、指示の言葉の写しだったもの", {
        応答: text,
      });
    }
    const speechEchoNotice =
      speechEchoes > 0
  ...
  /**
   * 退けた関係を、退けた記録から外す（作者の裁定、2026-09-23）。
   *
   * 外すのは「次の抽出や外部AIの提案が足してよい」と許すことで、
   * **関係そのものは戻さない**（戻すなら関係欄に書いて保存する——
   * そうしても記録から外れる）。記録は作者の操作でしか変えない欄なので、
   * `autoGenerated` は触らない（掘り下げメモの削除と同じ）。
   */
  private async handleForgetRejectedRelation(
    id: string,
    target: string,
    relation: string
  ): Promise<void> {
    ...
    const updated = forgetRejectedRelation(character, target, relation);
    if (updated === character) {
      // 別の窓で先に外された場合など。読み直して今の状態を見せる
      await this.reloadAfterSave(
        "character",
        id,
        `「${target}=${relation}」は、退けた記録にありません。`
      );
      return;
    }
    await this.persist("character", updated);
    await this.reloadAfterSave(
      "character",
      id,
      `「${target}=${relation}」を退けた記録から外しました。次の抽出では、本文にあれば足されます。`
    );
  }
...
function referenceLines(
  ...
  const heldFields = changes && currentOf
    ? [...new Set(changes.map((change) => change.field))]
        .map((field) => ({
          field,
          held: heldChangesOfField(changes, field, currentOf(field)),
        }))
        .filter((entry) => entry.held.length > 0)
    : [];
  return [
    /*
      **要確認を、確定した変化より先に置く**（作者の裁定、2026-09-23）。
      根拠（本文の引用）が無いので本体の値は変えていない変化で、作者の判断を
      待っている。認めれば本体へ入る。誤りなら下の「変化」の行の
      「誤りを落とす」で落とす（値を消す操作は1か所にしか置かない）
    */
    ...heldFields.map(({ field, held }) => ({
      label: `要確認（${field}）`,
      value: `${describeChangeValues(held)} — 根拠（本文の引用）が無いので、本体の値は変えていません`,
      action: {
        label: "変化として認める",
        field,
        kind: "confirmChanges" as const,
      },
    })),
    ...
    /*
      **落とした値を見せる**（黙って消したことにしない。2026-09-26 深夜）。
      値ごとに［誤りの印を外す］を添える（2026-09-26）。外すと次の抽出や
      外部AIの提案がその値をまた出せるようになるだけで、本体は変わらない。
      本体に入れたいなら、これまでどおり欄へ直接書く（落とした値の記録は、
      抽出がその値を入れないためのもので、作者の手入力は止めない）
    */
...
function conflictValueActions(
  ...
  return {
    valueActions: values.map((value) => {
      return {
        label: `「${shortValueLabel(value)}」は誤り（落とす）`,
...
function rejectedValueLines(
  ...
  return [...byField.entries()].map(([field, entries]) => ({
    ...
    value: entries
      ...
      .join("、"),
    /*
      **印を外す札を値ごとに置く**（作者の裁定、2026-09-26）。行は項目ごとに
      1行へまとめたままにし、札だけを値の数だけ並べる（食い違いの行と同じ形）。
      送る値は記録そのまま——札の文字（頭だけ見せた長い値）から切り出さない
    */
    valueActions: entries.map((entry) => ({
      label:
        entries.length > 1
          ? `「${shortValueLabel(entry.value)}」の誤りの印を外す`
          : "誤りの印を外す",
      title:
        `「${entry.value}」の誤りの印を外します。本体の値と食い違いの記録は戻しません。` +
        "次の抽出でこの値がまた出れば、食い違いとして並びます",
      field,
      value: entry.value,
      kind: "undoRejectedValue" as const,
    })),
  }));
}

/** 札に載せる値。長い値は頭だけ見せ、全文は指を載せると出す */
function shortValueLabel(value: string): string {
  const chars = [...value];
  return chars.length > VALUE_ACTION_LABEL_CHARS
    ? `${chars.slice(0, VALUE_ACTION_LABEL_CHARS).join("")}…`
    : value;
}

/**
 * 性格の面の行（作者の裁定、2026-09-24 夜。設計書6.5.10）。
 *
 * - 「性格の面」：いまも成り立つ面を、見えた話と並べる。面が2つ以上あれば
 *   「作中の変化にする」を添える（変わる前と後は押した先で選ばせる）
 * - 「過去の性格の面」：作者が「変わった」と決めた面。「面に戻す」を添える
 *
...
function personalityFacetLines(
  ...
  const active = activeFacets(facets);
  const past = facets.filter((facet) => facet.supersededBy);
  const lines: DetailView["reference"] = [];
  if (active.length >= 2 || (active.length === 1 && past.length > 0)) {
    lines.push({
      label: "性格の面",
      value: describePersonalityFacets(active),
      ...(active.length >= 2
        ? {
            action: {
              label: "作中の変化にする",
              field: "personality",
              kind: "markPersonalityChange" as const,
            },
          }
        : {}),
    });
  ...
  if (past.length > 0) {
    lines.push({
      ...
      value: past
        .map(
          (facet) =>
            `${describePersonalityFacets([facet])}→ ${facet.supersededBy ?? ""}`
        )
        .join("、"),
      action: {
        label: "面に戻す",
        field: "personality",
        kind: "restorePersonalityFacet" as const,
      },
    });
  }
  return lines;
...
interface PersonalityFacetPick extends vscode.QuickPickItem {
  /** どの面か。表示の文字列ではなく面の値そのものを持つ */
  value: string;
}

/** 面の選択肢。見えた話と根拠を添える（取り違えを見抜く手掛かり） */
function personalityFacetPicks(
  facets: readonly PersonalityFacet[]
): PersonalityFacetPick[] {
  return facets.map((facet) => ({
    label: facet.value,
    description:
      facet.chapters.length > 0 ? formatChapters(facet.chapters) : "それ以前",
    detail: facet.evidence ? `根拠：${facet.evidence}` : undefined,
    value: facet.value,
  }));
}
...
type PanelMessage =
  ...
  /** 退けた関係を、退けた記録から外す（2026-09-23） */
  | {
      type: "forgetRejectedRelation";
      kind: SettingsKind;
      id: string;
      target: string;
      relation: string;
  ...
  /** 「誤りとして落とした値」の印を1件外す（作者の裁定、2026-09-26） */
  | {
      type: "undoRejectedValue";
      kind: SettingsKind;
      id: string;
      /** どの項目の記録か */
      field: string;
      /** 外す値（記録されていたまま。前後の空白も揃えない） */
      value: string;
    }
  ...
  /** 根拠が無いので本体へ入れていない変化を、作者が認める（2026-09-23） */
  | {
      type: "confirmChanges";
      kind: SettingsKind;
      id: string;
  ...
  /**
   * 性格の面のうち、作中で変わったものを作者が決める（2026-09-24 夜）。
   * どの面からどの面へかは、押した先で選ばせる（`field` は常に性格）
   */
  | {
      type: "markPersonalityChange";
      kind: SettingsKind;
      id: string;
      field: string;
    }

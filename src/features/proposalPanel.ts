...
export interface RecordUpdateViewItem {
  ...
  source: string;
  /**
   * 出どころの印（`PendingUpdateSource`。設計書6.87.18）。
   *
   * **「外部AIの提案をまとめて確かめる」は、画面の文言（`source`）ではなく
   * この印で見分ける。** 文言で見分けると、言い回しを直した日に黙って効かなくなる
   */
  origin?: string;
...
type IssuesMessage = {
  ...
  canApplyAll: boolean;
  /**
   * 「外部AIの提案をまとめて確かめる」を出すか（設計書6.87.18）。
   * 設定資料の更新に、外部AIから来た未判断の案があるときだけ
   */
...
type IncomingMessage =
  ...
  /**
   * 外部AIの提案だけをまとめて確かめて承認する（設計書6.87.18）。
   * `drops` は「まとめて適用」と同じ（✕ を付けた葉を落とす）
   */
  | { type: "approveExternal"; drops?: RecordDropKeys[] }
  /** 別の分類へ切り替える */
...
export class ProposalPanel implements vscode.WebviewViewProvider {
  ...
  private postItems(): void {
    ...
    const message: IssuesMessage = {
      ...
      canApproveExternal: updateMode && hasExternalPending(this.recordUpdates),
  ...
  private async handleMessage(message: IncomingMessage): Promise<void> {
    switch (message.type) {
      ...
      case "applyAll":
        ...
        return;
      case "approveExternal":
        await this.approveExternalRecordUpdates(message.drops);
        return;
      case "selectCategory":
  ...
  /**
   * 外部AIの提案だけを、まとめて確かめて承認する（設計書6.87.18）。
   *
   * **束ねる側では何も迂回しない。** 1件ずつの「反映する」と同じ
   * `applyRecordUpdateById` へ流すので、作者のデータの保護（`authorNotes`・
   * `autoGenerated: false`・`authorLocked`・6.18 の話数の扱い）はそのまま効く。
   * ✕ を付けた葉も同じく落とす。
   *
   * **押す前に中身を見せる。** 名前と理由を1件ずつ並べたモーダルを出し、
   * 「承認する」を押したときだけ進める。
   */
  private async approveExternalRecordUpdates(
    drops?: readonly RecordDropKeys[]
  ): Promise<void> {
    const targets = externalBulkTargets(this.recordUpdates);
    if (targets.length === 0) {
      void vscode.window.showInformationMessage(
        "外部AIからの、まだ判断していない提案はありません。"
      );
      return;
    }
    const approve = "承認する";
    const confirm = await vscode.window.showWarningMessage(
      `外部AIからの提案 ${targets.length} 件を、まとめて承認しますか？`,
      {
        modal: true,
        detail:
          `${bulkApprovalDetail(targets)}\n\n` +
          "作者が書いたメモや、作者が固定した呼び方は変わりません（1件ずつ反映するときと同じ守りが効きます）。" +
          "差分は一覧で1件ずつ見られます。✕ を付けた項目は落とします。",
      },
      approve
    );

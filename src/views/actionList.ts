...

  {
    ...
    entries: [
      ...
      // なく、登録済みの作品そのものの決めごとなので、分類の直下に置く
      {
        ...
        requiresWork: true,
        detail:
          "同じ世界・同じ人物を書いている作品どうしを、ゆるくつなぎます。" +
          "**つなげるのは、同じフォルダーに並んでいる作品だけです。**" +
          "相手の人物・場所・能力・組織の**名前と読み仮名だけ**を借りて、" +
          "用語の色分け・ルビ・IME辞書・表記ゆれの材料に足します。" +
          "設定資料の抽出で同じ名前が出たときは、**候補として並べるだけ**で、" +
          "勝手に同じ人物にはしません。" +
          "**中身（紹介など）は読まず、相手の資料も書き換えません。**",
      },
      /*
      ...
      },
      {
        ...
        items: [
          ...
          // 「初めて使う人」の最初の壁になっていた
          {
            ...
            requiresWork: false,
            detail:
              "投稿サイトからダウンロードしたZIPを、そのまま作品にします。" +
              "展開・作品フォルダーの用意・登録までを一度に行います。" +
              "**作品名は about.txt のタイトルから採ります**（次の画面で直せます）。" +
              "キャッチコピー・紹介文・ジャンル・タグが書いてあれば、設定資料の下書きとして置きます。",
          },
        ],
      },
      {
      ...
      },
      {
        ...
        items: [
          ...
          // 2026-09-21）。機械を行き来する前に、これだけ押せばよい
          {
            ...
            requiresWork: false,
            detail:
              "**開いている原稿を保存してから**、記録してGitHubへ送ります。" +
              "**別の機械で書く前に、これだけ押せば済みます。**" +
              "「作品をすべて同期」は未保存があると止まりますが、" +
              "こちらは先に保存します。",
          },
      ...
      */
      {
        ...
        */
        detail:
          "取り込んだばかりの作品に対して、**AIでできることを順に全部走らせます**" +
          "（設定資料の抽出・各話あらすじ・プロットの逆算・紹介文・キャッチコピー・" +
          "校正一式・章立ての提案）。" +
          "**既にあるもの（設定資料・あらすじ・プロット）の段は飛ばします。**" +
          "走らせる段と送る量は、始める前に1回だけ確認します。" +
          "**本文は1文字も書き換えません**（指摘は「提案」パネルに溜まります）。" +
          "終わったら、何ができたかを1枚の文書にまとめて開きます。" +
          "途中で中止すると、残りは走りません。",
      },
      {
        kind: "action",
      ...
      // 作品ごとの「ターゲット読者診断」とは別物なので、並べない
      {
        ...
        usesAI: false,
        detail:
          "9つの質問で、**あなた自身が読者として何を求めるか**を測ります" +
          "（作品の宛先ではありません。**あなたが読むときの話**です）。" +
          "答えると、**その作品のターゲット読者との違い**が出ます——" +
          "自分に効くものと、その作品の読者に効くものは別々に動くためです。" +
          "**上下はありません。**どの読み方にもその読み方なりの良さがあり、効く相手が違うだけです。" +
          "**答えは作者の手元にだけ残り、作品のファイルにもGitHubにも入りません。**" +
          "いつでもやり直せますし、消せます。",
      },
      {
        kind: "action",
      ...
      },
      {
        ...
        items: [
          ...
          // 診断から遠いところにあると、先に何を押すのか分からない
          {
            ...
            requiresWork: true,
            detail:
              "**狙っている読者層と、書けているものの実態**を1枚に並べます。" +
              "11の読者層それぞれとの一致度と、読者層を**広げる・絞る**の" +
              "向かう先が出ます。先に「ターゲット読者診断」を済ませてください。" +
              "狙いは紙の「狙い」の欄へ作者が書きます" +
              "（作り直しても残ります）。**AIは使いません。**" +
              "作り直すたびに控えが残り、推移として並びます。本文は書き換えません。",
          },
          ...
          // 宛先を決める道具で、こちらはその宛先と実績・嗜好を並べる1枚
          {
            ...
            requiresWork: true,
            detail:
              "**すでに書けたもの・書きたいもの・読者が読みたいもの**を、" +
              "1枚に並べて出します。重なっているところと、離れているところが見えます。" +
              "材料は原稿と台帳から数えるので、**AIは使いません**。" +
              "**品定めではありません**——上下はなく、効く相手が違うだけです。" +
              "本文も台帳も書き換えません。",
          },
          {
            kind: "action",
          ...
          // 「矛盾を検知」としばらく並行させるので、隣に並べて見比べられるようにする
          {
            ...
            usesAI: true,
            detail:
              "AIには「本文に何が書いてあるか」だけを取り出させ、" +
              "食い違いの判定は拡張機能が機械的に行います。" +
              "見つかった候補だけを、もう一度AIが1件ずつ確かめます。" +
              "**本文は書き換えません。** 設定資料が無くても実行できます" +
              "（本文どうしの食い違いを見ます）。",
          },
          ...
          */
          {
            ...
            requiresWork: true,
            detail:
              "保存日数（既定3日）を過ぎたAIの指摘を、置き場から消します。" +
              "**期限を過ぎた指摘は一覧から隠れているだけで、消えてはいません**" +
              "——押す前に何件消えるかをお見せして、確認してから消します。" +
              "**本文にも設定資料にも触りません。** 消すと元に戻せません。",
          },
      ...
      */
      {
        ...
        items: [
          ...
          // 揃える」操作で、作者が押したときだけ動く
          {
            ...
            requiresWork: true,
            detail:
              "この作品の本文の改行コード（LF / CRLF）を見て、多数派と違うものを" +
              "まとめて揃えます。**他のツールで扱うときの不具合を防ぐため**の操作で、" +
              "改行が違うファイルは、他のツールで開いたときに全行が変更扱いになったり、" +
              "行の位置がずれたりします。" +
              "**本文は1文字も変えません**（文字コードも末尾の改行もそのまま）。" +
              "**作者が押したときだけ変えます**——保存や同期で勝手に揃えることはありません。",
          },
          ...
          */
          {
            ...
            detail:
              "外部のAIがこの作品を扱うときの決まりを書いた指示書を、作品フォルダーへ置きます。" +
              "**どの相手向けかを選べます**（複数可）。中身はどれも同じで、置き先と頭の数行だけが違います。" +
              "MCPサーバーの登録（`.mcp.json` など）も一緒に書きます。" +
              "既にあるファイルは、退避するかどうかをお尋ねしてから置き換えます。" +
              "**置いただけでは、外部AIはまだ1文字も読めません**" +
              "（読ませるかどうかは「外部AIの利用を許可する／取り消す」で決めます）。",
          },
          {
            kind: "action",

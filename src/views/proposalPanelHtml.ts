...
/*
  **「準備できた」と伝える**（0.45.2、設計書6.11.9）。

  この面は作者の知らないところで組み直される（窓の大きさを変える、
  下段のタブを行き来する）。組み直されると script は最初から走り直し、
  画面は空に戻るが、**組み直しの直前・直後に送られた中身は、聞く人が
  いないまま捨てられる**。窓を元の大きさに戻すと分類タブの行がまるごと
  消え、印だけが前の数のまま残っていたのはこれである（実機、2026-09-11）。

  聞ける状態になってから声をかけ、送り直してもらう。**いちばん最後に
  置く**——これより上で組み立てに失敗していたら、送ってもらっても
  受け取れない。
*/

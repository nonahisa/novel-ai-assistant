...
#compose .ellipsis {
  ...
  /* **三点リーダが欧文へ落ちるのを防ぐ**（実機の報告、2026-08-29）。
     選んだ書体が「…」を持たないと欧文へ落ち、欧文の三点リーダは下寄りの
     字形なので、中央指定をしても沈んで見える。

     **作者の書体を先頭に置く**（実機の報告、2026-09-15。0.64.4で直した）。
     以前は和文明朝を名指しで固定していたが、**作者が別の明朝
     （BIZ UDMincho）を選ぶと、そこだけ游明朝になって字面が変わる**——
...
/* **ダッシュが欧文へ落ちるのを防ぐ**（作者の実機報告、2026-08-29
   ...
   **作者の書体を先頭に置く**（実機の報告、2026-09-15。0.64.4で直した。
   三点リーダと同じ理由——名指しで固定すると、別の明朝を選んだ作者の
   本文の中で**そこだけ書体が変わる**）。
  ...
  function paintCounts() {
    ...
    if (footToday !== null) {
      /*
        ...
        **「この話で」と断る**——数えているのはこのファイルで今日書いた量で、
        作品全体の今日の量ではない。「今日 0字」の隣に「2日連続で目標に
        届きました」が並ぶと、書いていないのに届いたと読めた（別の話で
        書いた分で届いていた。ノートPCの実機確認、2026-09-25）
      */
      parts.push(

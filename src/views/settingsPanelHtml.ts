  ...
  window.addEventListener("message", function (event) {
    ...
    switch (message.type) {
      ...
      case "focus":
        ...
        /*
          **本文の用語から来たときは、一覧を畳んで出す**（作者の依頼、
          2026-08-28）。本文の隣に並ぶ狭い幅を一覧に取られると、
          肝心の資料が読めない。

          **畳むのはここだけ。** 作者が自分で一覧を開いたら、次に用語から
          来るまではそのままにする（メニューなど他の入口から開いたときは、
          この印が付かないので、これまでどおり一覧が出る）。
        */
        if (message.collapseList) setCollapsed(true);
        renderTabs();
        renderList();
        renderDetail();

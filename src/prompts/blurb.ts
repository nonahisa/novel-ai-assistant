 ...
 */
export const BLURB_TEMPERATURE = 0.5;

/**
 * キャッチコピーの温度。**紹介文とは別の値である。**
 *
 * 案を出させるので、いちばん揺らす。同じシステムプロンプトを使うが、
 * 仕事が違えば温度も違う——1つにまとめると、どちらかが巻き添えになる。
 */
export const CATCHPHRASE_TEMPERATURE = 0.9;
...
- **最初の1文に、作品のいちばんの引き（核）を置くこと。**
  投稿サイトの一覧では紹介文の後半が省略されて表示されるため、
  前置きから書き始めると、読者には核が届かない。
...
- **最初の数語に、その案の核となる言葉を置くこと。**
  一覧では文の後半が切れて表示されることがあり、後半に核があると読者に届かない。

 ...
 *   **コードが決めた枠・項目1つだけ**）。AIが選ぶ型でも、場面の3点と目標の
 *   文字数はコードが「次に尋ねる1点」として割り込む（1.3）
 ...
 * - 1.3: AIが選ぶ型（着想・場面・結末）でも、コードが割り込んで決めた1点
...
/**
 * AIが選ぶ型へ、コードが割り込んで1点を決めたときの見出し（`nextGuidedPoint`）。
 * **答えとして返ってきたら中身が無い**（検算の止める言葉に入れてある）
 */
const GUIDED_LABEL = "次に尋ねる1点";

/** AIが選ぶ型の共通の一文。コードが割り込んだ回（場面の3点・目標の文字数）はそれだけを尋ねさせる */
const GUIDED_RULE = `ただし依頼文に「${GUIDED_LABEL}」があるときは、その1点だけを尋ねること。`;
...
> = {
  idea: {
    ...
    pick: `着想のいちばん強いところから、いま決めると話が一番広がる1点を選ぶこと。尋ねる順は決まっていません。${GUIDED_RULE}`,
    flow: `人物・世界の仕組み・舞台に加えて、目標の文字数と大きな流れ（事件の並び・山場・どんでん返し）も、この問答の中で決めていきます。
   世界の仕組みばかりを続けて掘らないこと。主人公（誰の目で見る話か）・構成・大きな流れのうち、まだ決まっていないものへも移ること。
   決まったことが5つを超えても、大きな流れがまだ決まっていなければ、次はそれを尋ねること。`,
  },
  scene: {
    intro: "作者には書きたい場面が1つあります。その場面を起点に、話を外へ広げる問いを1つずつ出します。",
    // 場面の直前・至る理由・その後は、コードが「次に尋ねる1点」で順に渡す（26b が直前を尋ねなかった）
    pick: `最初は、場面の直前・場面に至る理由・場面のその後を、依頼文の「${GUIDED_LABEL}」で1つずつ尋ねます。そのあとは、その場面から話を外へ広げる1点を選ぶこと。${GUIDED_RULE}`,
    flow: `場面の中の細部（台詞・描写・直前の心境・きっかけの小さな仕組み）は、1回尋ねたら続けて尋ねないこと。次は場面の外——人物がなぜそこにいるか・話の始まり・場面のあとの展開・話の終わり・目標の文字数——へ移ること。
   決まったことが5つを超えても、話の始まりか終わりがまだ決まっていなければ、次はそれを尋ねること。`,
  },
  ending: {
    intro: "作者は話の終わり方を決めています。その結末から逆算して、そこへ至るのに必要な出来事を1つずつ尋ねます。",
    pick: `結末の直前から話の始まりへ向かってさかのぼり、その結末が成り立つのに欠かせない1点を選ぶこと。${GUIDED_RULE}`,
   ...
   決まったことが5つを超えても、話の始まりがまだ決まっていなければ、次はそれを尋ねること。`,
  },
  structure: {
...
export interface PlotDialoguePromptInput {
  ...
  frame?: PlotFrame;
  /**
   * コードが決めた次の1点（型に当てはめる・項目を順に埋める）。AIが選ぶ型でも、
   * コードが割り込んだ回（場面の3点・目標の文字数。`nextGuidedPoint`）は渡す
   */
...
export function buildPlotDialoguePrompt(input: PlotDialoguePromptInput): string {
  ...
  const fixedLabel =
    style === "structure" ? "次に埋める枠" : style === "fields" ? "次に埋める項目" : GUIDED_LABEL;
  const ask = fixed

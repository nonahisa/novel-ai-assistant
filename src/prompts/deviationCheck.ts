 ...
 *
 * 1.2（残課題8）：指摘が無いときは空の配列で返し、そこで終えるよう書いた。
 * 実機（2026-09-22、さくらのAI `preview/gemma-4-31B-it`）で「（該当箇所なし）」
 * の要素を1件入れたあと、空白だけの行が出力上限まで続いたため。**この指示の
 * 言葉はそのまま返ってくる前提**で、埋め草の要素は検算
 * （`core/deviationValidation.ts` の `isFillerDeviation`）でも落とす。
 */
export const DEVIATION_CHECK_VERSION = "1.2";
...
- **指摘が無いときは "deviations" を空の配列にして、そこで出力を終えてください。**
  「該当なし」を表す要素を入れないこと。閉じ括弧のあとに空白や改行を続けないこと。

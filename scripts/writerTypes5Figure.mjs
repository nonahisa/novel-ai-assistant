/*
  ...
  **中身は1文字も変えない。** タイプ名も番号も位置も、作者の元の図のまま。
  ...
  ——白地、Yu Gothic。並べて貼っても浮かない。

  **見出しと解説は入れない**（作者の指示、2026-09-19）。note の側で見出しを
  付けるので、図の中に持つと二重になる。そのぶん余白を詰め、図を真ん中へ
  寄せてある——文字を消しただけだと、上に大きな空きが残る。
*/
...
/*
  軸の交点。**横の中心より少し左**に置く。
  右端には「読者志向」（4文字）、左端には「低」（1文字）が付くので、
  真ん中に置くと右の余白だけが狭く見える。
*/
const CX = 575;
const CY = 469;
...
// 見出しと解説は入れない（理由は冒頭に書いた）。図そのものだけを置く

...
**条件のはっきりした片づけは `scripts/cleanup.mjs` が起動時に自動で行う**（`.claude/settings.json` の `SessionStart`）。この担当は、判断が要る片づけのときだけ呼ばれる。

- 自動で行うもの：①一覧（`git worktree list`）に載っていない、または枝が無くなった `agent-*` の作業場（連結を外してから消す）、②開くフォルダーがもう無い起動設定（「たゆたう鉛_確認用」は残す）、③起動から12時間以上たった試験用プロセス（親が生きている MCP サーバーは残す）
- 記録は `.git/novelai-cleanup.log`。**呼ばれたら、まずこの記録と `node scripts/cleanup.mjs --dry-run` の結果を見る**（自動で消せずに残したもの・理由が出ている）
- 自動では行わないもの（この担当の仕事）：取り込み済みの担当の**枝**の削除、スクラッチパッドの写し、Chrome のタブ、配布物、12時間たっていないが使い終わったと分かっているプロセス、自動の片づけが「消せず」と記録したもの

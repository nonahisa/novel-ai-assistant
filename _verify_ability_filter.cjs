...
// src/core/groundedEvidence.ts
function isGroundedInChunk(appellations, evidence, chunkText) {
  const normalizedAppellations = appellations.map((appellation) => normalizeForComparison(appellation ?? "")).filter((appellation) => appellation.length > 0);
  if (normalizedAppellations.length === 0) return false;
  const normalizedChunk = normalizeForComparison(chunkText);
  if (!normalizedAppellations.some(
    (appellation) => normalizedChunk.includes(appellation)
  )) {
    return false;
  }
  return evidenceSegments(evidence).some(
    (segment) => normalizedChunk.includes(segment)
  );
}
function normalizeForComparison(text) {
  return text.replace(/<0x[0-9A-Fa-f]{2}>/gu, "").replace(/[\s　]/gu, "");
}
function evidenceSegments(evidence) {
  if (!evidence) return [];
  return evidence.split(/[\r\n。！？!?]+/u).map(
    (segment) => segment.replace(/^[「『"'“”‘’（(\s…]+|[」』"'“”‘’）)\s…]+$/gu, "")
  ).filter((segment) => segment.length >= 4).map(normalizeForComparison).filter((segment) => segment.length >= 4);
}
function chaptersForChunk(chunk) {
...
// src/core/settingsExtractionValidation.ts
var MAX_NAME_LENGTH = 40;
var SENTENCE_PUNCTUATION = /[、。，,.！？!?；;：:\r\n]/u;
var PLACEHOLDER_NAME_PATTERN = /^(null|undefined|不明|なし|n\/?a|none|その他|特になし)$/i;
var DEMONSTRATIVE_PATTERN = /^(ここ|そこ|あそこ|どこ|この|その|あの)/u;
var ABILITY_GENERIC_TERMS = /* @__PURE__ */ new Set([
  "\u80FD\u529B",
  "\u529B",
  "\u30B9\u30AD\u30EB",
  "\u6280\u80FD",
...
var DESCRIPTIVE_NAME_PATTERN = /(?:できる|できた|られる|される|している|していた|いる|ある|ない|だけ|など|ため|こと|ようだ|そうだ|らしい)$/u;
var NON_ABILITY_SUFFIX_PATTERN = /(?:制度|機関|組織|協会|ギルド|商会|会社|法律|条例|規則|契約)$/u;
var GENRE_NAMES = /* @__PURE__ */ new Set([
  "\u30D5\u30A1\u30F3\u30BF\u30B8\u30FC",
  "\u30CF\u30A4\u30D5\u30A1\u30F3\u30BF\u30B8\u30FC",
  "\u30ED\u30FC\u30D5\u30A1\u30F3\u30BF\u30B8\u30FC",
  "\u4F1D\u5947",
...
function validateExtractedAbilities(raw, chunk, abilityTerm) {
  const accepted = [];
  const rejected = [];
...
function validateExtractedLocations(raw, chunk) {
  const accepted = [];
  const rejected = [];
  if (!Array.isArray(raw)) return { accepted, rejected };
  const chapters = chaptersForChunk(chunk);
...
function normalizeExtractedAbilitySystem(raw) {
  if (!isRecord2(raw)) return void 0;
  const abilityTerm = typeof raw.abilityTerm === "string" ? raw.abilityTerm.trim() : "";
  const description = typeof raw.description === "string" ? raw.description.trim() : "";
  const rules = cleanStringArray(raw.rules);
  const usableLabel = abilityTerm.length > 0 && abilityTerm.length <= 10 && !SENTENCE_PUNCTUATION.test(abilityTerm) && !PLACEHOLDER_NAME_PATTERN.test(abilityTerm) && !isGenreName(abilityTerm) ? abilityTerm : null;
  if (!usableLabel && !description && rules.length === 0) return void 0;
  return {
...
function normalizeExtractedAbility(raw) {
...
function normalizeExtractedLocation(raw) {
  const name = typeof raw.name === "string" ? raw.name.trim() : "";
  return {
    name,
    aliases: cleanStringArray(raw.aliases).filter((alias) => alias !== name),
...
function isSpecificAbilityName(name, abilityTerm) {
...
function isGenreName(value) {
  const parts = value.split(/[\/／、,・]/u).map((part) => part.trim().toLowerCase()).filter(Boolean);
...
function buildCharacterExtractPrompt(input) {
  ...
  const knownAbilities = input.knownAbilityNames && input.knownAbilityNames.length > 0 ? input.knownAbilityNames.join("\u3001") : "\uFF08\u307E\u3060\u767B\u9332\u3055\u308C\u3066\u3044\u307E\u305B\u3093\uFF09";
  const knownLocations = input.knownLocationNames && input.knownLocationNames.length > 0 ? input.knownLocationNames.join("\u3001") : "\uFF08\u307E\u3060\u767B\u9332\u3055\u308C\u3066\u3044\u307E\u305B\u3093\uFF09";
  const abilityTermNote = input.abilityTerm ? `\u3053\u306E\u4F5C\u54C1\u3067\u306F\u80FD\u529B\u3092\u300C${input.abilityTerm}\u300D\u3068\u7DCF\u79F0\u3057\u307E\u3059\u3002abilitySystem.abilityTerm \u306B\u306F\u540C\u3058\u8A9E\u3092\u4F7F\u3063\u3066\u304F\u3060\u3055\u3044\u3002` : `abilitySystem.abilityTerm \u306B\u306F\u3001**\u4F5C\u54C1\u4E16\u754C\u306E\u4E2D\u3067\u80FD\u529B\u3092\u7DCF\u79F0\u3057\u3066\u3044\u308B\u8A9E**\u3092\u3001\u672C\u6587\u306E\u8868\u8A18\u306E\u307E\u307E\u5165\u308C\u3066\u304F\u3060\u3055\u3044\u3002
...
// _verify_ability_filter.mts
var WORK_DIR = "C:/Users/nonah/Documents/\u6559\u79D1\u66F8\u30C1\u30FC\u30C8";
var OLLAMA = "http://localhost:11434";
async function main() {
  const entries = (await fs.readdir(WORK_DIR, { withFileTypes: true })).filter((e) => e.isFile() && /\.txt$/i.test(e.name));
  const chunks = [];
  for (const e of entries) {
    const raw = await fs.readFile(path2.join(WORK_DIR, e.name));
    ...
    chunks.push(...splitIntoChunks(path2.join(WORK_DIR, e.name), meta.body, p.chapterStart, p.chapterEnd, { maxChars: decideChunkSize(131072) }));
  }
  chunks.sort((a, b) => (a.chapterStart ?? 999) - (b.chapterStart ?? 999));
  const abilities = /* @__PURE__ */ new Set(), locations = /* @__PURE__ */ new Set();
  const rejected = {};
  ...
  for (const chunk of chunks.slice(0, Number(process.env.LIMIT ?? 8))) {
      ...
      body: JSON.stringify({
        ...
        messages: [
          ...
          { role: "user", content: buildCharacterExtractPrompt({
            ...
            chapterLabel: `\u7B2C${chunk.chapterStart}\u8A71`,
            knownCharacterNames: [],
            knownAbilityNames: [...abilities],
            knownLocationNames: [...locations],
            abilityTerm: abilityTerm ?? void 0
          }) }
        ],
    ...
    if (!parsed) continue;
    const sys = normalizeExtractedAbilitySystem(parsed.abilitySystem);
    if (sys?.abilityTerm && !abilityTerm) abilityTerm = sys.abilityTerm;
    const a = validateExtractedAbilities(parsed.abilities, chunk, abilityTerm);
    for (const x of a.accepted) abilities.add(x.data.name);
    const l = validateExtractedLocations(parsed.locations, chunk);

...
このリポジトリの版：**0.23.0**（配布のまとまり。既知の穴の修正・LM Studioの導入案内・指摘の「再チェック」・実機確認で見つかった不具合の修正を含みます）。Marketplace に出ている版はこれより遅れることがあります。
...
| 書く | 話数・文字数の自動集計、原稿用紙換算、執筆量の記録とグラフ、用語ハイライト。**縦書きで読み書きできる画面。** ルビと傍点を範囲選択で入れる |
...
| 書き出す | 設定資料集のMarkdown、IME辞書（Google日本語入力・MS-IME・ATOK）、投稿サイトごとのルビ・傍点記法への変換 |
...
起動したウィンドウの左端アクティビティバーに、キーボードと万年筆のアイコン（**統合小説執筆環境**）が追加されます。開くと2つのビューが並びます。
...
**「執筆AI支援 → その他支援 → 縦書きで開く」**、または本文を右クリックして「縦書きで開く」。開いている原稿を、縦書きの画面で開き直します。
...
| **書く／読む** | 下の表のとおりです |
| **並べる** | 打つ面と組み上がりを左右に並べます。**打ちながらルビや色分けを見られます**（組み上がりの側は、カーソルのある行を追いかけます） |
| **ルビ／傍点** | 選んだ文字に振ります |
...
| 面 | 見えるもの |
|---|---|
| **書く** | ふつうに打てます。ルビは `{漢字|かんじ}`、傍点は `{{強調}}` のまま見えます |
| **読む** | ルビが振り仮名として、傍点が圏点として出ます。**登場人物・場所・能力・組織が色分けされ、押すとその設定資料が開きます** |

右クリックからは、ルビ・傍点・投稿サイト用のコピー・AIへの相談ができます。用語の上で右クリックすると、その設定資料を開く項目が増えます。

**保存はふつうのエディタとまったく同じです。** 保存も、元に戻す（Ctrl+Z）も、外部ツールで書き換えられたときの読み直しも、VS Code がそのまま行います。文字コードと改行も変わりません。

**この画面が既定になることはありません。** `.md` を開いてもふつうのエディタのままです。縦書きで見たいときだけ開いてください（VS Code の「エディターを再度開く」からも選べます）。

書体は明朝体です（設定 `novelai.manuscriptEditor.fontFamily` で変えられます）。

> **0.19.2 で、日本語入力（IME）が壊れていたのを直しました**（入力中にカーソルが飛ぶ・文字が二重に入る・変換が途中で止まる）。原稿が画面へ送り返されるとき、変換の最中に打っている面へ入れ直していたためです。**まずは作品のコピーでお試しください。**

---

## ルビと傍点

**本文を範囲選択して右クリック → 「ルビを振る」「傍点を付ける」**（操作メニューの「執筆AI支援 → その他支援」からも同じことができます）。漢字の直後にカーソルがあれば、選ばなくてもその漢字を拾います。

**Markdown（`.md`）のファイルだけで使えます。** `.txt` のままなら、下記の「本文を .md にする」で変えてください。
...
**右クリック → 「投稿サイト用に変換してコピー」**（操作メニューの「執筆AI支援 → その他支援」からも同じ）。サイトごとの記法に直したものをクリップボードへ入れます。**原稿は書き換えません。**
...
- **締切からの逆算、提供先別の設定資料エクスポート、伏線の追跡は未実装です。**

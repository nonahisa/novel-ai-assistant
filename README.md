...
**書くこと以外、引き受けます。** 小説の執筆で積み重なる「書く前」と「書いた後」の仕事——プロットづくり、設定資料づくり、見直し、投稿の準備、電子書籍・PDFでの出版——を、**原稿には手を触れないAI**と一緒に片づけるための拡張機能です。

> **※現在テスト中です。** 動作の確認（実機テスト）が済んでいない機能が含まれます。大切な原稿は、必ずバックアップ（GitHub同期など）を取ったうえでお使いください。

このリポジトリの版：**0.38.0**（0.37.1〜0.37.4を含む配布版）。Marketplace に出ている版はこれより遅れることがあります。
...
- **「書き上げた作品を、電子書籍や紙の形にしたい」**
  → EPUBエディターで表紙・目次・挿絵・奥付を確かめながら電子書籍を組めます。印刷用のPDFも作れます。
- **「どのサイトの何話まで投稿したか、分からなくなる」**
  → 投稿サイトごとの投稿状況・作品情報・ランキングを手元に記録できます（サイトへの自動アクセスは一切しません）。
...
| 場面 | できること |
|---|---|
| 始める | プロットから、または本文から新規作品を作る。既存のフォルダやGitHubの作品を登録する。**プロットモード**（目次と話の見取り図を横に並べて書く画面）で構想を練る |
...
| 整える | プロットを本文から逆算する。各話あらすじ・作品紹介文・キャッチコピー・**更新告知文**を作る。**章立てのAI提案**。各話の盛り上がりを感情曲線で見る |
| 直す | 誤字脱字・表記ゆれの検知、推敲の提案、プロットからの逸脱と矛盾の検知（**過去の話の本文まで見ます**）、**伏線の配置・回収の検知**。表記ゆれは**どちらに揃えるかをAIに訊ける**。**指摘は1件ずつ確認して適用する。** 方言や口癖は「直さない語」に登録して守れる |
| 相談する | 開いている画面についてAIに日本語で質問する。**質問に近い場面を作品全体から探して答えます。** 相談で決めた設定は、**承認つきで設定資料へ反映**できる。この拡張機能の使い方も聞ける |
| 持ち運ぶ | GitHubで複数の環境を行き来する。**1つのリポジトリ（書庫）に複数の作品**を置ける。競合は並べて見比べて解決する |
| 投稿する | 投稿サイトごとのルビ・傍点記法への変換とコピー。**投稿サイトごとの投稿状況・作品情報・ランキングの記録**（手入力のみ——サイトへの自動アクセスはしません） |
| 出版する | **EPUBエディター**——表紙・目次・挿絵・奥付・人物紹介を、プレビューで確かめながら電子書籍に組む。**PDF出力（印刷用の組版）** |
| 書き出す | 設定資料集のMarkdown（**編集部向け・イラスト発注向けなど提供先別**にも）、IME辞書（Google日本語入力・MS-IME・ATOK） |
...
各メニューの右クリック最上段には「**他のメニューを閉じる**」があり、1つのメニューに集中できます（戻すのは「すべてのメニューを出す」）。
  ...
  執筆AI支援        相談・プロット（直下）／校正・校閲／広報支援／原稿づくり／投稿・書き出し
  資料管理          資料抽出／設定資料閲覧
...
**本文を範囲選択して右クリック → 「ルビを振る」「傍点を付ける」**（詳細メニューの「執筆AI支援 → 原稿づくり」からも同じことができます）。漢字の直後にカーソルがあれば、選ばなくてもその漢字を拾います。
...
**右クリック → 「投稿サイト用に変換してコピー」**（詳細メニューの「執筆AI支援 → 投稿・書き出し」からも同じ）。サイトごとの記法に直したものをクリップボードへ入れます。**原稿は書き換えません。**
...
すでに `.md` になっている本文には、「執筆AI支援 → 投稿・書き出し → **投稿サイトのルビを取り込む**」を使います。何件変わるかを先に見せます。
...
- 「執筆AI支援 → 原稿づくり → **本文を .md にする**」（作品まるごとか、開いている1件かを選べます）
...
抽出したJSONから読むための資料集を書き出すのが「**設定資料集を出力**」（執筆AI支援 → 投稿・書き出し）です。JSONを手直ししたあとや、人物をまとめたあと・更新を反映したあとに実行すると、上の5つのMarkdownが最新の内容になります。AIは呼びません。
...
「執筆AI支援 → 原稿づくり → **シーンメモを開く**」で、原稿エディタの横にパネルが開きます。作品の全メモを並べ、押すと該当行へ。**「次へ」「戻る」**で話をまたいで順に飛べます（コマンド `novelai.nextSceneMemo`／`novelai.prevSceneMemo` にキーを割り当てられます）。カーソルに追従して近いメモが光り、この話だけ・タグ・文字で絞り込めます。「済みにする」でメモ行を消せます（取り消しは Ctrl+Z か「復元」）。作品一覧の話の右には「TODO 3」のような件数の印が出ます。
...
| `novelai.epub.ornamentFolder` | `""`（指していない） | EPUBの飾り（目次・中表紙・奥付）を追加で読み込むフォルダーの絶対パス。この中の `*.svg` が、**すべての作品**の「飾り」の選択肢に出ます。作品ごとに足すときは `設定/書籍/飾り/` に置いてください（そちらはGitで同期されます）。組み込みの飾りと同じ名前のファイルは使いません |

...
このリポジトリの版：**0.8.1**（実際に使って見つかった不具合の修正）。Marketplace に出ている版はこれより遅れることがあります。
...
「AI設定 → 相談で使う検索 → **意味検索（ベクトルDB）の準備**」から入れられます。入れると、**言い換えで聞いたときにも見つかりやすく**なります（例：「妬ましさを感じる場面」→「嫉妬」の場面）。

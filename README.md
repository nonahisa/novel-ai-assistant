...
- **操作メニュー**：押せる操作の一覧です。コマンド名を覚えていなくても、ここから選べます。「資料」「整える」「校正・校閲」「書き出す」「同期」「AI設定」「困ったとき」に分かれており、作業の流れで辿れます。作品を登録していない間は、作品を必要とする操作は出ません。

  分類は**最初はすべて閉じています**。開いた分類は次に起動したときも開いたままになるので、よく使う分類だけ開いておけます。
...
**あとからどちらへも移れます。** プロットは操作メニューの「**プロットを作る**」でいつでも足せます（すでに書きかけのプロットがあれば、上書きせずそのまま開きます）。本文は「新規話数ファイルを追加」で足せます。
...
作品を右クリック →「設定資料を抽出」。全話を解析して、登場人物・能力・組織・場所・世界観をまとめて取り出します。
...
抽出したJSONから読むための資料集を書き出すのが「**設定資料集を出力**」です。JSONを手直ししたあとや、人物をまとめたあと・更新を反映したあとに実行すると、上の5つのMarkdownが最新の内容になります。AIは呼びません。

### 設定資料パネル

作品を右クリック →「設定資料を開く」。抽出した登場人物・能力・組織・場所・世界観を1枚の画面で扱えます。左が一覧、右が選んだ1件の詳細です。
...
AI処理の記録は「**ログを表示**」で開けます。あわせて `作品フォルダ/.aiwriter/logs/actions.log` にも保存されるので、VS Codeを閉じたあとでも読み返せます（Gitの同期対象外です）。
...
- プロット支援、推敲、矛盾検知は今後の機能です。世界観の抽出とGitHub同期（未取得の通知・取り込み・送信・競合の解決）は実装済みです。
...
- フェーズ1（一部完了）：AIプロバイダ連携・設定資料抽出（登場人物・能力・場所・組織・世界観）は実装済み。プロットモードは未実装
- フェーズ2（完了）：用語ハイライト、各話あらすじ・サブタイトル生成、作品紹介文、キャッチコピー提案
- フェーズ3（一部完了）：誤字脱字検知とAI指摘パネルは実装済み。推敲・逸脱検知・矛盾検知は未実装
- フェーズ4（一部完了）：執筆量の記録・グラフ・目標・話ごとの文字数一覧は実装済み。ルビ機能とGit自動同期は未実装（同期は手動の取り込み・送信まで実装済み）
- フェーズ5：設定資料エクスポート

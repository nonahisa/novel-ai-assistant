...
このリポジトリの版：**0.18.3**（説明書の更新）。Marketplace に出ている版はこれより遅れることがあります。
...
| 書く | 話数・文字数の自動集計、原稿用紙換算、執筆量の記録とグラフ、用語ハイライト。ルビと傍点を範囲選択で入れる |
...
**本文を範囲選択して、「執筆AI支援 → その他支援 → ルビを振る／傍点を付ける」でルビと傍点を入れられます。** 漢字の直後にカーソルがあれば、選ばなくてもその漢字を拾います。
...
「執筆AI支援 → その他支援 → **投稿サイト用に変換してコピー**」で、サイトごとの記法に直したものをクリップボードへ入れます。**原稿は書き換えません。**

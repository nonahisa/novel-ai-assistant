...
このリポジトリの版：**0.26.10**（更新告知文・AIが実際に読める長さを測る・本文を溢れさせない関所・シーンメモ・年表。0.26.0＝VSIX配布版）。Marketplace に出ている版はこれより遅れることがあります。
...
「拡張機能の設定 → AI → **AIが実際に読める長さを測る**」（AIを使います）。無害な詰め物の先頭と末尾に合言葉を置いて送り、両方が返るかで「入力が黙って切り捨てられていないか」を確かめ、実際に読める上限を割り出します。さくらのAI Engine のように上限を公開していないサービスや、LM Studio で読み込んだ長さの確認に使えます。結果は通知に出て、「設定に書く」を押したときだけ設定へ書きます。有料AIでは実行前に見込み（最大で数十円）を出して確認します。
